...
**属性变化时间线**:
一个节点按会话链列出的行，每行一个会话：`sesno · 时间 · E3D 保存人（user）· 保存备注（comment）· 属性变化数 · 几何影响`，「所有子节点」下再加「单元 n」（该会话动了几个最小交付单元）。它回答「谁在哪一版改了什么」；任意两版的净差看属性对比。行的属性变化数取自 e3d-io 会话差分的逐属性 before / after，不生成几何；user / comment 来自会话页本身。
_Avoid_: 属性历史对比（那是 A / B 两版的逐项对比，不是时间线）、会话列表（漏掉了两列影响）、版本列表（泛指时）
