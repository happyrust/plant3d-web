...
- **三维查看器新增「E3D 外观」渲染模式——复刻 AVEVA E3D 3.1 SGL DX11 的画法，设置里可切** (2026-09-20 ~ 2026-09-21，分支 `codex/e3d-sgl-look-prototype`，13 个提交)
  - 依据是对 `sglDx11.dll`（E3D 2.10 / 3.1）的逆向：DXBC 反汇编 + 反编译逐句对照，报告在仓外 `D:\ida_scratch\plant3\render\`；web 侧完整记录见 `docs/rendering/e3d-sgl-look-prototype.md`。新模块 `src/viewer/e3dLook/`，接入 `DTXMaterial`（program key v16）/ `DTXLayer` 与 `ViewerPanel`，演示页 `/e3d-look-demo.html`（仅 dev）。
  - **实体着色**（`SglLookMaterial` / DTX SGL 分支）：单盏头灯 Blinn-Phong + 环境反射，`rgb = c·(Ka + Kd·N·L) + Ks·(N·H)^Kse·N·L + Kr·env(reflect)`；环境立方体贴图就是从 `sglDx11` `.data` 里抓出来的那张 128² 灰度 DDS（`public/texture/e3d/sgl31-envcube/`）。两套预设：**出厂 E3D 3.1**（PML `gphviewopt.default` → `{0.7, 0, 0, 0.8, 0}`，边线 / 伪阴影 / 渐变 / 4× 抗锯齿全开，grey→白渐变）与 **本机真机**（C++ 默认 `{0.5, 0.8, 0.3, 0.35, 64}`，全关，纯灰）。
  - **元素颜色**：逆向出的 PDMS 颜色表 365 项（`pdmsColourTable.ts`，`pdms:lightgrey` 这类颜色引用）；新显示主题 `e3dFactory`（出厂 autocolour 关 → 所有元素 lightgrey #bdbdbd，`ThemeConfig.baseMaterial` 新字段）；选中 / CE 按 `gphcolopt.default`（CE yellow、其余 highlight white、无描边，`SelectionManager.primarySelectionColor`）；DTX 颜色在 SGL 分支按 sRGB 还原。
  - **后处理链**（`SglLookPipeline`）：面法线 / 线性深度 MRT（背景哨兵 1e18，效果参与标记编进法线长度：`EnhancedEdgesTranslucent` / `EnhancedEdgesHandles` / `PseudoShadowsHandles`）→ HBAO（3.1 硬编码 R 392.73 mm / 8 方向 / 4 步 / 30° / Attenuation 0.2）→ 深度感知模糊（12 px，锐度每帧按深度范围 = 16 / (范围/2)）→ HLR 边线（dxbc 逐句：右/下邻居、|Δd| > 50、|dot| < 0.9999、|N·N′| < 0.6）→ **HLR 去杂**（`HLRDeclutter`：2×2 窗口 Σ×1/9、(min,max) = (0, 0.3)，单根线保留 74% 黑度、密集处淡掉）→ `colour × HLR × AO` 合成 + 背景纵向渐变（D2D 参数 t 0.233→0.9）；抗锯齿 = 颜色通道 4× MSAA + HLR 2×2 超采样 Σ/N，可选 FXAA。
