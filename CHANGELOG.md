...
- **校审单流转到 JH / SH / PZ 后三维视口不再空白：审核面板「只显示任务构件」按单元子树亮、没装就不动场景；第二份 `useModelGeneration` 对已画好的 BRAN 不再误报「1 个 refno 没有几何记录」** (2026-09-23，issue [#84](https://github.com/happyrust/plant3d-web/issues/84)，线上 `123.57.182.243` 单据 `FORM-C8049FC8F784`)
  ...
  - 改法：`useDbnoInstancesDtxLoader.resolveDtxRefnosByUnitRefno`（`resolveDtxObjectIdsByUnitRefno` 的 refno 版）+ `DtxCompatScene.getSubtreeRefnos(refnos)`（自己有对象才算 + owner 链落到它的已装构件；占位不算）；新纯函数 `components/review/taskModelFilter.ts` `applyTaskModelFilter(scene, taskRefnos)`：任务 refno 先解成子树里真正装进场景的 refno，一个都没装 → `task-not-loaded` **不动场景**，否则藏全部、亮「单元 + 子树」、飞 `getSubtreeAABB`；`ReviewPanel.filterModelByTask` 改走它。`useModelGeneration.showModelByRefno`：「已存在」分支 `getAABB` 空时再试 `getSubtreeAABB`（有成员就飞成员盒）；收口新增一档 `loadedObjects===0 && skipped>0 && mesh404===0 && 没几何的只有请求的根自己` → 「已加载 (gen-model)」、记 `loadedRoots`、回 `true`（`GenModelV1LoadTotals` 多一格 `noGeoRefnos`）；`checkRefnoExists` 归一 refno。
  - 验证：vitest 新 `taskModelFilter.test.ts` 5 例（单元亮子树 / 占位不算不动场景 / 叶子与多 refno 并集 / 盒回落 / 空场景）、`DtxCompatScene.getSubtreeAABB.test.ts` 4 → 7（`getSubtreeRefnos`：管道 BRAN 含自己、HVAC 支管只回成员且占位不算、叶子 / 空 / 非 refno）、`useModelGeneration.genModelV1.test.ts` +2（第二实例对已画好的 BRAN 回 `true`、飞成员盒、记已加载、无「没有几何记录」；范围里只有根自己仍照报）；`ReviewPanel.vue` 改动按 AGENTS.md 跑 5 套双胞胎面板回归；type-check 新增 0；eslint 触及文件 0 错误。
  - 上线复验（2026-09-24，`0dec493d` 经 deploy-ubuntu.yml run 35951124846 部到 `123.57.182.243`，`version.json 0dec493d` / `index-DARS-7AE.js`）：JH（jd）与 SH（sh）各以模型中心直领的 token 打开 `FORM-C8049FC8F784`（`workflow/sync?query`：`currentNode=jd`、`models=["24384_24935"]`），场景 47 条状态里有几何的只有 BEND `24384_24936` + STRT `24384_24939`，两者可见、0 被藏，`getSubtreeRefnos([BRAN])` 正解成这两个、`getSubtreeAABB` 有盒而 `getAABB` 为空；模型树日志 `Model already loaded: 24384/24935`（修前是 `Auto-load failed`），页面无「没有几何记录」文案、console error / warn 0；同一场景手动回放旧两步（藏全部 → 只亮 BRAN 键）可见几何立刻为空，按新逻辑亮子树即恢复。图与读数见 `docs/verification/review-task-model-filter-2026-09-24/README.md`。
- **节点版本视图撞上连 `model/versions` 都没有的旧服务端时照实说「要新版服务端」，不再露裸「HTTP 404 Not Found」** (2026-09-23，线上 `123.57.182.243` 版本查询 / 对比报 404 的排查结论)
  - 现场：线上前端是 main `a38bd354`（09-23 01:49 UTC 部的），后端却是 `deploy/backend-20260918` 分支的 gen-model `0.1.27+gc04a9e558`（09-18 10:09 构建，分叉点 `d8a0c4f9e`）——它有 `model/history/generate | query`，但 **没有** `model/versions`（`ef2284f12`）、`element/versions`（`6cb37395f`）、`node/versions`（`a382b2cf3`）、`element/attribute-history | attribute-diff`、`node/diff-summary` 六条路由（`git grep` 该分支 0 命中；线上 `GET /api/v1/{model,element,node}/versions` 都是无信封 404，`route_count 71` 与本机相同只是巧合）。前端对同一夹具在本机 `:8022`（`0.1.30+g0b2bf527b`）上 `model-version-compare` 4 例 + `node-version-view` 4 例 **8 passed 32.1 s**，代码本身没问题；线上面板却只写「HTTP 404 Not Found」，看不出是服务端缺路由。
  - 改法：`versionSource.listVersions` 第一页撞上无信封 404（`isMissingRoute`）→ 抛 `ModelVersionRouteUnavailableError('model/versions')`（服务端自己答的带信封码 404，如 `REFNO_NOT_FOUND`，原样抛）；`listElementVersions` 的回落 `unitOnlyTimeline` 因此也一并转成它。面板 `loadVersions` 的 catch 对这一类错误直说「服务端还没有 `model/versions`：版本查询 / 对比要带该路由的新版服务端，当前站点接的后端还是旧构建；后端升级前这里查不出任何版本」。`ModelVersionSource.listVersions` 端口注释同步。**真修法是换后端**：把 gen-model-refactor（≥ `a382b2cf3`；本机 `:8022` 跑的 `0b2bf527b` 六条都有）并进 `deploy/backend-20260918` 再走 `deploy-linux-ubuntu.yml`——那是后端仓的部署动作，本条不做。
  - 验证：vitest `versionSource.test.ts` 21 → 23（无路由 → `ModelVersionRouteUnavailableError('model/versions')` 且不含「HTTP 404」/ 带信封的 404 原样抛；`element/versions` + `model/versions` 双缺 → 同一个错误、各打 1 次）、`ModelUnitVersionComparePanel.test.ts` 24 → 25（错误框含「服务端还没有 model/versions」「新版服务端」、不含「HTTP 404」，时间线 / 范围开关不渲染，`listVersions` 未再打），四个版本相关 test 文件 78 过；eslint 触及 5 文件 0；type-check 新增 0（当前基线外的 3 条在 `instanceMapping.test.ts`，`d0cd5971` 带进来的既有账，与本条无关）。线上现状与改后对照（Playwright 开 `http://123.57.182.243/?unit_refno=24384_26480&compare_autorun=1`：`element/versions` 404 → `model/versions` 404 → 线上包错误框「HTTP 404 Not Found」；改后工作树 dev 直连同一线上后端 → 「服务端还没有 model/versions：…」，`pageerror` 都是 0）见 `docs/verification/model-version-compare-gen-model-v1-2026-09-18/README.md` §9 + `online-old-backend-0923/` 两图。教程 §1 前置条件补这一档。
- **左侧工具栏「测量」下拉 / 「查看工具设置」弹层开着时容器一变矮就重新落位，不再按旧容器顶出去** (2026-09-22，issue [#83](https://github.com/happyrust/plant3d-web/issues/83)，#80–#82 线上验收时顺手发现)
  - 之前 #81 的落位只在 toggle 打开那一刻算一次；弹层开着拖控制台分隔条 / 缩窗 / 折叠面板，容器矮了也不重算——线上 `a9358f09` cua 真机：高容器开的「查看工具设置」缩到 431 px 容器后仍 `max-height 476`、顶出容器 60 px；「测量」下拉同理不翻上、末两项重新被裁。
  - 现在 `.viewer-panel-container` 那个已有的 `ResizeObserver` 回调里顺手 `replaceOpenLeftToolbarPopups()`：哪个弹层开着就重跑它的 `placeLeftMeasureMenu` / `placeLeftSettingsPopup`。两个 place 函数量高度改用新导出的 `utils/dropdownPlacement.ts` `measureNaturalHeight(el)`——同步清掉元素上一轮的内联 `max-height` / `overflow-y` 量自然高度再恢复（同一 JS 任务内，不掉帧不闪），否则会拿已限高的高度越算越小 / 误判放得下。`resolveDropdownPlacement` 纯函数不动。
  - 验证：vitest `dropdownPlacement.test.ts` 6 → 8 例（`measureNaturalHeight` 清内联限高量、量完恢复 / 本无限高不多出样式）；eslint 触及两个 utils 文件 0，`ViewerPanel.vue` 只剩第 28 行既有那条；type-check 触及文件 0 新增。Playwright headless 拦截换包（本地 build 对线上后端，`docs/verification/pms-3d-review-integration-e2e.md` §6.3.1 做法）A/B：视口 1692×839（容器 567）打开 → 缩到 1692×659（容器 430）——线上旧包 `index-Bq92dgMb.js`：菜单仍 `top-0` 337→555 超出容器底 42 px、弹层仍 `max-height 475` 23→498 顶出 60 px；修后 build：菜单 `bottom-0` 155→373、弹层重算 `max-height 407` 91→498 都在容器内，放大回去菜单又向下开、弹层回 475；`pageerror` 0。图 `docs/verification/viewer-overlay-container-clip-2026-09-22/resize-83-{menu,settings}-{online,local}.png`，README §2.7。
