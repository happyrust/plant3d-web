...
- The submit button text and success copy already encode the split: external mode says "保存提资单数据" / "已保存到提资单，流程流转由外部系统继续处理", while internal/manual mode says "创建并提交提资单" / "已提交到校审流程". See `src/components/review/InitiateReviewPanel.vue:501`, `src/components/review/InitiateReviewPanel.vue:515`, `src/components/review/InitiateReviewPanel.vue:522`.
...
- `InitiateReviewPanel` only auto-submits after create when `externalWorkflowMode` is false; the code path is explicit: `if (!isExternal) await userStore.submitTaskToNextNode(task.id, '发起提资')`. See `src/components/review/InitiateReviewPanel.vue:603`.
