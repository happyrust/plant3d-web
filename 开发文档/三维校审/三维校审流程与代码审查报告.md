...
- 当前已验证的 live smoke path：`http://127.0.0.1:3101/` 进入 app -> 点击项目卡片进入主界面 -> 切换到 reviewer 身份 `李审核员`（当前界面显示为 backend 用户 `user-002`）-> 打开 reviewer inbox（`panel.reviewerTasks`）-> 看到真实任务卡片 `测试提资单-1767928364` -> 点击 `处理校核` 进入 reviewer active task。当前 reviewer workbench 已真实展示同一任务的标题、模型 `test-model`、当前节点 `校核`，以及标准 workflow 入口 `提交到审核` / `驳回到设计`。
- 当前 live boundary：本轮只安全验证到真实 workflow dialog 打开。`驳回到设计` 能弹出要求必填原因的标准驳回对话框，`提交到审核` 能弹出提交备注对话框；为了不意外改变本地任务状态，本轮未执行最终确认提交/驳回。
