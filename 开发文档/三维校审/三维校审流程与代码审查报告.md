...
> 生成时间：2026-03-06  
> 范围：`plant3d-web` 前端三维校审主流程（入口、编校审、审核、流转、同步、通知）
...
### 步骤 1：发起编校审（设计人员）
- 触发：`InitiateReviewPanel` 中点击“创建编校审单”。
- 前端操作：
...
- 节点定义：`sj -> jd -> sh -> pz`。
- 前端操作：
  - 确认流转到下一节点：`submitTaskToNextNode(taskId, comment)`。
  - 确认驳回流转到指定节点：`returnTaskToNode(taskId, targetNode, reason)`。
  - 并行存在兼容“直接通过/驳回”按钮：`updateTaskStatus(approved/rejected)`。
- 调用接口：
...
- 当前已验证的 live smoke path：`http://127.0.0.1:3101/` 进入 app -> 点击项目卡片进入主界面 -> 切换到 reviewer 身份 `李审核员`（当前界面显示为 backend 用户 `user-002`）-> 打开 reviewer inbox（`panel.reviewerTasks`）-> 看到真实任务卡片 `测试编校审单-1767928364` -> 点击 `处理校对` 进入 reviewer active task。当前 reviewer workbench 已真实展示同一任务的标题、模型 `test-model`、当前节点 `校对`，以及标准 workflow 入口 `确认流转至审核` / `确认驳回流转`。
- 当前 live boundary：本轮只安全验证到真实 workflow dialog 打开。`确认驳回流转` 能弹出要求必填原因的标准驳回对话框，`确认流转至审核` 能弹出确认提交流转对话框；为了不意外改变本地任务状态，本轮未执行最终确认提交/驳回。
- 对应自动化保障：`src/composables/useReviewStore.websocket.test.ts` 现在覆盖真实 `setCurrentTask(...)` hydration，断言 reviewer workbench 在 active task 加载后保留 `formId`、附件、confirmed records 与 workflow history；本轮 live 浏览器证据则把 smoke path 进一步推进到真实 inbox -> active task -> workflow dialog 打开。
