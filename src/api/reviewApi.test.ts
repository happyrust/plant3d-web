...
import {
  authGetToken,
  authVerifyToken,
  getReviewEvidenceSaveErrorMessage,
  ...
  shouldRetryAttachmentUploadAsJson,
  normalizeReviewTask,
...
describe('reviewApi base url defaults', () => {
  ...
  it('decides the JSON upload fallback by semantics, not by one framework wording', () => {
    // axum 现行文案：即便状态码与预期不符也必须继续兜底，否则 standalone 上传直接断掉
    expect(shouldRetryAttachmentUploadAsJson(
      400,
    ...
    // 换过措辞（框架升级/换实现）仍能识别：只要说明「Content-Type 要 application/json」
    expect(shouldRetryAttachmentUploadAsJson(
      415,
      '{"error":"expected content-type application/json, got multipart/form-data"}',
    )).toBe(true);
    expect(shouldRetryAttachmentUploadAsJson(
      400,
      'Unsupported Content_Type; this endpoint only accepts APPLICATION/JSON',
    )).toBe(true);

    // plant-model-gen 的真实校验失败不能被误判成需要兜底，否则会把 50MB 文件再用 base64 打一遍
    expect(shouldRetryAttachmentUploadAsJson(
      413,
      '{"success":false,"error_message":"文件超过 50MB 上限（当前 62.0MB）"}',
    )).toBe(false);
    expect(shouldRetryAttachmentUploadAsJson(
      415,
      '{"success":false,"error_message":"不支持的文件类型「.exe」","operation":"review.attachments.validate_ext"}',
    )).toBe(false);
    expect(shouldRetryAttachmentUploadAsJson(
      403,
      '{"success":false,"error_message":"仅任务发起人可编辑附件"}',
    )).toBe(false);
    expect(shouldRetryAttachmentUploadAsJson(504, '')).toBe(false);
  });
  ...
  it('does not re-send the file as base64 when the server rejected the file itself', async () => {
    ...
    await expect(reviewAttachmentUpload(
      'task-1',
      new File([new Uint8Array([37, 80, 68, 70])], 'big.pdf', { type: 'application/pdf' }),
    )).rejects.toThrow(/413/);
  ...
  it('formats review save availability errors without hiding explicit business errors', () => {
    expect(getReviewEvidenceSaveErrorMessage(new ReviewApiHttpError({
      status: 504,
      statusText: 'Gateway Timeout',
      message: 'HTTP 504 Gateway Timeout',
    }), '保存失败')).toBe('校审服务暂时不可用，未保存证据仍已保留，请稍后重试');

    expect(getReviewEvidenceSaveErrorMessage(new ReviewApiHttpError({
      status: 503,
      statusText: 'Service Unavailable',
      message: 'HTTP 503 Service Unavailable',
    }), '保存失败')).toBe('校审服务正在恢复，请稍后重试');
    ...
    expect(getReviewEvidenceSaveErrorMessage(new ReviewApiHttpError({
      ...
      message: '业务校验失败',
      errorMessage: '业务校验失败',
      responseBody: { error_message: '业务校验失败' },
    }), '保存失败')).toBe('业务校验失败');

    expect(getReviewEvidenceSaveErrorMessage(new Error('POST /api/review/records 超时'), '保存失败'))
      .toBe('保存请求超时，当前批注未丢失');
    ...
    expect(getReviewEvidenceSaveErrorMessage(new ReviewApiHttpError({
      ...
      message: 'HTTP 500',
      responseBody: { error_message: '校审数据库上下文切换失败：review.ensure_context 超时(10000ms)' },
    }), '保存失败')).toBe('保存超时，请重试；服务器繁忙或网络较慢，你的批注仍保留');
