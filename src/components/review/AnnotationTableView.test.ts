...
describe('AnnotationTableView', () => {
  ...
  it('1. 渲染 3 行批注 + 表头', async () => {
    ...
    const rows = host.querySelectorAll<HTMLElement>('[role="row"][data-testid^="annotation-table-row-"]');
  ...
  it('2. 空数组显示 empty state · 不显示表头和行', async () => {
    ...
    const rows = host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]');
  ...
  it('3. 单击行立即 emit select-annotation', async () => {
    ...
    const row = host.querySelector<HTMLElement>('[role="row"][data-testid^="annotation-table-row-"]');
    row?.click();
    await nextTick();
  ...
  it('6. 搜索输入 · 300ms debounce 后过滤行数', async () => {
    ...
    // 还在 debounce 窗口内
    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]')).toHaveLength(3);
  ...
  it('7. 错误类型筛选 · select 变更后行数变化', async () => {
    ...
    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]')).toHaveLength(1);

    destroy();
  });

  it('7c. 被筛空时区分于"没有批注"，并可一键清除筛选', async () => {
    const items = [
      createItem({ id: 'g1', severity: 'general' }),
      createItem({ id: 'g2', severity: 'general' }),
    ];
    const { host, destroy } = mountTable({ items });
    await nextTick();
    ...
    expect(empty?.textContent).toContain('没有符合筛选条件的批注');
    expect(empty?.textContent).toContain('当前共 2 条批注');
    expect(empty?.textContent).not.toContain('当前范围内还没有可处理的批注');

    (host.querySelector('[data-testid="annotation-table-empty-clear-filters"]') as HTMLButtonElement).click();
    await nextTick();

    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]')).toHaveLength(2);
    expect(host.querySelector('[data-testid="annotation-table-empty"]')).toBeNull();
    expect(host.querySelector('[data-testid="annotation-table-clear-filters"]')).toBeNull();

    destroy();
  });

  it('7d. 真正没有批注时保留原空状态，不提示清除筛选', async () => {
    const { host, destroy } = mountTable({ items: [] });
    await nextTick();

    const empty = host.querySelector('[data-testid="annotation-table-empty"]');
    expect(empty?.getAttribute('data-empty-reason')).toBe('no-data');
    expect(host.querySelector('[data-testid="annotation-table-empty-clear-filters"]')).toBeNull();
  ...
  it('11. > pageSize 时显示分页 footer · 点 next 跳下一页', async () => {
    ...
    // 第一页 5 行
    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]')).toHaveLength(5);
  ...
  it('13. Compact 模式（<640px）· 表格变卡片 role="listitem" · 表头消失', async () => {
    ...
    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]').length).toBe(2);
    ...
    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]').length).toBe(0);
  ...
  it('14. Medium 模式（640–960px）· 隐藏 description · 保留表头', async () => {
    ...
    expect(host.querySelectorAll('[role="row"][data-testid^="annotation-table-row-"]').length).toBe(1);
  ...
  it('14b. 表格 ARIA 结构：grid 拥有表头行与 rowgroup，数据行由 gridcell 组成', async () => {
    const { host, destroy } = mountTable({
      items: [createItem({ id: 'aria-1' }), createItem({ id: 'aria-2' })],
      currentAnnotationId: 'aria-1',
      currentAnnotationType: 'text',
      renderExpandedRow: true,
    });
    await nextTick();
    ...
    // rowgroup / row / columnheader / gridcell 脱离 grid 就不成立，读屏软件会整块丢弃表格结构
    const owned = host.querySelectorAll('[role="rowgroup"], [role="row"], [role="columnheader"], [role="gridcell"]');
    expect(owned.length).toBeGreaterThan(0);
    owned.forEach((el) => expect(el.closest('[role="grid"]')).toBe(grid));

    const headerRow = host
      .querySelector<HTMLElement>('[data-testid="annotation-table-sort-index"]')
      ?.closest('[role="row"]');
    expect(headerRow?.querySelectorAll('[role="columnheader"]')).toHaveLength(5);

    const dataRow = host.querySelector<HTMLElement>('[data-testid="annotation-table-row-aria-1"]');
    expect(dataRow?.getAttribute('role')).toBe('row');
    expect(dataRow?.querySelectorAll(':scope > [role="gridcell"]')).toHaveLength(5);

    const expandedRow = host.querySelector<HTMLElement>('[data-testid="annotation-table-expanded-aria-1"]');
    expect(expandedRow?.getAttribute('role')).toBe('row');
    expect(expandedRow?.querySelector('[role="gridcell"]')?.getAttribute('aria-colspan')).toBe('5');

    // 表头行与展开行都不带 tabindex，方向键才不会停在它们上面
    expect(headerRow?.hasAttribute('tabindex')).toBe(false);
    expect(expandedRow?.hasAttribute('tabindex')).toBe(false);

    // Compact 档是 list 语义，aria-selected 在 listitem 上无效，改用 aria-current
    const root = host.querySelector<HTMLElement>('[data-testid="annotation-table-view"]');
    fireContainerResize(root!, 400);
    await nextTick();

    const card = host.querySelector<HTMLElement>('[data-testid="annotation-table-row-aria-1"]');
  ...
  it('14c. 可排序列的 aria-sort 随排序状态转移', async () => {
    const { host, destroy } = mountTable({
      items: [createItem({ id: 'sort-1' }), createItem({ id: 'sort-2' })],
    });
    await nextTick();

    const headerOf = (testid: string) => host
      .querySelector<HTMLElement>(`[data-testid="${testid}"]`)
      ?.closest('[role="columnheader"]');

    // 默认按 status desc 排序
    expect(headerOf('annotation-table-sort-status')?.getAttribute('aria-sort')).toBe('descending');
    expect(headerOf('annotation-table-sort-severity')?.getAttribute('aria-sort')).toBe('none');

    host.querySelector<HTMLElement>('[data-testid="annotation-table-sort-severity"]')?.click();
    await nextTick();

    expect(headerOf('annotation-table-sort-severity')?.getAttribute('aria-sort')).toBe('descending');
    expect(headerOf('annotation-table-sort-status')?.getAttribute('aria-sort')).toBe('none');
