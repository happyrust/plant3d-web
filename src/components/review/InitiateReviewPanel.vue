...
import { buildReviewAttachments, restoreUploadedFilesFromAttachments } from './reviewAttachmentFlow';
...
// 恢复转换统一走 reviewAttachmentFlow.restoreUploadedFilesFromAttachments：
// 恢复条目必须是 status='success' 且带 serverAttachmentId/serverUrl，
// 否则下一次保存 buildReviewAttachments 会把历史附件整批裁剪掉（附件丢失回归）。
...
function applyRestoredTaskDraft() {
  ...
  selectedComponentRefno.value = null;
  uploadedFiles.value = restoreUploadedFilesFromAttachments(draft.attachments);
...
// 说明：这里不再有 syncTaskAttachments() 的重复网络写入。
// 附件上传/删除的任务投影一致性由后端 /api/review/attachments 接口维护；
// 任务创建请求仍携带完整附件列表用于兼容。

/** 保存前的附件状态门禁：上传中/待上传需等待，失败需处理，防止用户误以为附件已保存 */
function getAttachmentSubmitBlockReason(): string | null {
  const files = uploadedFiles.value;
  if (files.some((f) => f.status === 'uploading')) {
    return '附件正在上传中，请等待上传完成后再保存';
  }
  if (files.some((f) => f.status === 'error')) {
    return '存在上传失败的附件，请重试或移除后再保存';
  }
  // 有上传链路（formId/taskId）时 pending 会自动开始上传，等待完成即可；
  // 无链路（本地手动模式）时 pending 文件会在任务创建后统一上传，不阻塞。
  if (canAutoUploadAttachments.value && files.some((f) => f.status === 'pending')) {
    return '附件待上传，请稍候（已自动开始上传）';
  }
  return null;
}
...
async function handleSubmit() {
  ...
  const attachmentBlockReason = getAttachmentSubmitBlockReason();
  if (attachmentBlockReason) {
    notification.value = {
      type: 'error',
      message: '附件尚未就绪',
      details: attachmentBlockReason,
    };
    return;
  }
  ...
  try {
    ...
    if (hasPendingUploads) {
      ...
      // 创建后补传的附件由后端接口直接维护任务投影；
      // 若仍有失败附件，提示用户在恢复的单据中重试，不阻断已完成的保存。
      const failedUploads = uploadedFiles.value.filter((f) => f.status === 'error');
      if (failedUploads.length > 0) {
        notification.value = {
          type: 'error',
          message: `有 ${failedUploads.length} 个附件上传失败`,
          details: failedUploads
            .map((f) => `${f.name}：${f.errorMessage || '上传失败'}`)
            .join('；'),
        };
      }
    }
