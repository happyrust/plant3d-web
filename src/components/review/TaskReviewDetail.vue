...
import type { ReviewAttachment, ReviewTask, WorkflowNode, WorkflowStep } from '@/types/auth';
...
/** 预览面板在主界面 dock 中，详情弹窗会遮挡它，因此打开预览后关闭弹窗 */
function handlePreviewAttachment(attachment: ReviewAttachment): void {
  const taskId = taskView.value.id?.trim();
  if (!taskId || !openReviewAttachmentPreview(taskId, attachment)) {
    emitToast({ message: '附件地址无效，无法预览', level: 'warning' });
    return;
  }
  emit('close');
}
