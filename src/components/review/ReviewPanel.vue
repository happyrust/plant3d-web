...
// dock 宿主宽度有限：normal 档的行内处理卡在 xl 断点会切成双列（最小 220+340px），
// 塞不进面板宽度，所以宿主传下来的 density 必须一路透到统一批注单。
...
// ─── 附件材料：补传 / 删除（后端强制权限，前端只做入口收敛） ───

/** 当前用户 ID：嵌入模式优先取可信 JWT 身份，本地模式回退到用户库 */
const currentReviewUserId = computed<string | null>(() => {
  const embedParams = persistedEmbedParams.value;
  if (embedParams) {
    const trusted = resolveTrustedEmbedIdentity(embedParams);
    const embedUserId = trusted?.userId
      || embedParams.verifiedClaims?.userId
      || embedParams.userId;
    if (embedUserId?.trim()) return embedUserId.trim();
  }
...
const canEditTaskAttachments = computed(() => {
  ...
  if ((task.currentNode || 'sj') !== 'sj') return false;
  const userId = currentReviewUserId.value;
  return !!userId && userId === task.requesterId?.trim();
});

const supplementFiles = ref<UploadedFile[]>([]);

watch(() => currentTask.value?.id, () => {
  supplementFiles.value = [];
});

/** 补传成功：后端已同步任务投影，刷新任务后把该条目从本地上传列表移除 */
async function handleSupplementUploadComplete(file: UploadedFile) {
  const taskId = currentTask.value?.id;
  if (taskId) {
    await refreshCurrentTask(taskId);
  }
  supplementFiles.value = supplementFiles.value.filter((item) => item.id !== file.id);
}

/**
 * 取消补传：请求体可能已经发完，服务端仍会落一条附件。
 * 刷新任务把这种"取消了却已入库"的条目显式呈现出来，用户可自行删除。
 */
async function handleSupplementUploadCancel() {
  const taskId = currentTask.value?.id;
  if (!taskId) return;
  await refreshCurrentTask(taskId);
  emitToast({ message: '已取消上传；若服务端已收完，附件列表中会出现该文件', level: 'info' });
}

async function handleDeleteAttachment(attachment: ReviewAttachment) {
  const taskId = currentTask.value?.id;
  if (!taskId) return;
  if (!window.confirm(`确认删除附件「${attachment.name}」？删除后不可恢复。`)) return;

  try {
    const result = await reviewAttachmentDelete(attachment.id);
    if (!result.success) {
      throw new Error(result.error_message || result.message || '删除失败');
    }
    emitToast({ message: `附件「${attachment.name}」已删除`, level: 'success' });
    await refreshCurrentTask(taskId);
  } catch (error) {
    emitToast({
      message: `删除附件失败：${error instanceof Error ? error.message : '请稍后重试'}`,
      level: 'error',
    });
  }
}
...
/**
 * 与视口浮动工具条共用 `toolStore.toolMode`，所以这里必须回显激活态：
 * 两处都能切换同一个工具，只有一处高亮会让人以为是两套独立开关。
 */
...
async function confirmCurrentData() {
  ...
  try {
    ...
    confirmError.value = resolveReviewEvidenceSaveErrorMessage(e, '确认当前数据失败');
  } finally {
...
/** 再点一次关闭，与浮动工具条的模式按钮保持同一套切换语义 */
function toggleAnnotationMode(mode: 'annotation' | 'annotation_cloud' | 'annotation_rect') {
  toolStore.setToolMode(toolStore.toolMode.value === mode ? 'none' : mode);
}

function startAnnotation() {
  toggleAnnotationMode('annotation');
}

function startCloudAnnotation() {
  toggleAnnotationMode('annotation_cloud');
}

function startRectAnnotation() {
  toggleAnnotationMode('annotation_rect');
}
...
/** e2e 造数据时统一挂到这个构件上，回放脚本按同一个 refno 断言 */
const AUTOMATION_ANNOTATION_REFNO = '24381/145018';
  ...
  if (isAutomation) {
    (window as unknown as Record<string, unknown>).__plant3dReviewerE2E = {
      addMockAnnotation(title?: string, description?: string) {
        ...
        // 不带 entityId/refnos 的批注挂不到构件上，统一批注单里根本不会出现
        toolStore.addAnnotation({
          id,
          entityId: AUTOMATION_ANNOTATION_REFNO,
          worldPos: [0, 0, 0],
          glyph: '1',
          refno: AUTOMATION_ANNOTATION_REFNO,
          refnos: [AUTOMATION_ANNOTATION_REFNO],
  ...
  <div class="flex h-full flex-col gap-3 overflow-y-auto p-3" data-panel="review" :data-density="props.density">
    ...
    <div v-if="currentTask" class="rounded-lg border border-slate-200 bg-slate-50"
      ...
      <div v-show="expandedTaskDetails" class="border-t border-slate-200 p-4 space-y-4">
        ...
        <!-- 关联文件（统一走 AssociatedFilesList，只读） -->
        <AssociatedFilesList :attachments="currentTask.attachments ?? []"
          title="关联文件"
          description=""
          empty-text="暂无关联文件"
          :previewable="true"
    ...
    <div v-if="currentTask || isExternalFormFocused"
      ...
      <AnnotationSheetWorkspace :items="scopedReviewerItems"
        ...
        @pick-elements="pickWorkspaceElements"
    ...
    <div class="rounded-lg border border-slate-200 bg-white">
      ...
      <div v-show="activeReviewTab === 'records'" class="p-4"
        ...
        <div class="mt-3 flex items-center justify-between border-t border-slate-100 pt-3">
          <div class="flex items-center gap-3 text-xs text-slate-500">
            <span>{{ reviewStore.confirmedRecordCount.value }} 批次</span>
            <span>批注 {{ reviewStore.totalConfirmedAnnotations.value }}</span>
            <span>测量 {{ reviewStore.totalConfirmedMeasurements.value }}</span>
          </div>
      ...
      <div v-show="activeReviewTab === 'attachments'" class="space-y-4 p-4">
        ...
        <!-- 附件列表（统一走 AssociatedFilesList；固定展示总数与空状态） -->
        <section v-if="currentTask"
          class="rounded-lg border border-slate-200 bg-slate-50 p-4"
          data-testid="review-workbench-attachments-zone">
          <AssociatedFilesList :attachments="currentTask.attachments ?? []"
            title="附件材料"
            description=""
            empty-text="暂无附件材料"
            :previewable="true"
            :editable="canEditTaskAttachments"
            @preview="previewAttachment"
            @delete="handleDeleteAttachment" />

          <!-- 补充附件：仅任务发起人在设计（sj）节点可见 -->
          <div v-if="canEditTaskAttachments"
            class="mt-3"
            data-testid="review-attachment-supplement-zone">
            <div class="mb-2 text-sm font-semibold text-slate-900">补充附件</div>
            <FileUploadSection v-model="supplementFiles"
              :max-files="10"
              :max-size="50"
              :task-id="currentTask.id"
              :form-id="currentTask.formId || null"
              :auto-upload="true"
              accept-types=".pdf,.dwg,.dxf,.xlsx,.xls,.csv,.doc,.docx,.png,.jpg,.jpeg"
              @upload-complete="handleSupplementUploadComplete"
              @upload-cancel="handleSupplementUploadCancel" />
          </div>
          <p v-else class="mt-3 text-xs text-slate-400">
            当前节点只读：仅任务发起人在设计（sj）节点可补传或删除附件。
          </p>
        </section>
      </div>
    </div>
