...
describe('runWithConcurrency', () => {
  ...
  it('keeps draining the queue after a worker rejects', async () => {
    const processed: number[] = [];

    await runWithConcurrency([0, 1, 2], 2, async (item) => {
      try {
        if (item === 1) throw new Error('upload failed');
        processed.push(item);
      } catch {
        // 单个上传失败不应中断其余文件，与 uploadFile 的错误处理保持一致
      }
    });

    expect(processed.sort((a, b) => a - b)).toEqual([0, 2]);
  });
