...
const reviewAttachmentDeleteMock = vi.hoisted(() => vi.fn(async () => ({ success: true })));
...
vi.mock('@/api/reviewApi', () => ({
  ...
  reviewAttachmentDelete: (...args: unknown[]) => (reviewAttachmentDeleteMock as any)(...args),
  reviewAttachmentUploadWithProgress: vi.fn(async () => ({
    success: true,
    attachment: { id: 'att-new', url: '/files/review_attachments/att-new.pdf' },
  })),
}));
...
const reviewAttachmentPreviewMock = vi.hoisted(() => ({
  ...
  open: vi.fn(() => true),
  buildDownloadUrl: vi.fn((attachment: { name?: string; url?: string }) => (
    `${attachment.url ?? ''}?download=1&name=${encodeURIComponent(attachment.name ?? '')}`
  )),
  active: { value: null as { attachment: { id: string } } | null },
}));
...
vi.mock('@/composables/useReviewAttachmentPreview', () => ({
  ...
  buildReviewAttachmentDownloadUrl: reviewAttachmentPreviewMock.buildDownloadUrl,
  activeReviewAttachmentPreview: reviewAttachmentPreviewMock.active,
}));
...
describe('ReviewPanel', () => {
  beforeEach(() => {
    ...
    reviewAttachmentPreviewMock.buildDownloadUrl.mockClear();
    reviewAttachmentPreviewMock.open.mockReset();
    reviewAttachmentPreviewMock.open.mockReturnValue(true);
    reviewAttachmentPreviewMock.active.value = null;
  ...
  it('renders the workbench sections and normalized context fields', async () => {
    ...
    // 流转区始终渲染：外部模式放状态提示条，内部模式放提交/驳回按钮
    const workflowZone = document.querySelector('[data-testid="review-workbench-workflow-zone"]');
    expect(workflowZone).not.toBeNull();
    expect(workflowZone?.textContent).toContain('外部流程模式');
    expect(document.querySelector('[data-testid="reviewer-annotation-sheet-container"]')).not.toBeNull();
  ...
  it('确认记录区展示批次统计与严重度分布，统一批注单展示对应批注行', async () => {
    sortedConfirmedRecords.value = [
      {
        id: 'record-dock-1',
        taskId: 'task-1',
    ...
    const recordsZone = document.querySelector('[data-testid="review-workbench-confirmed-records-zone"]');
    expect(recordsZone).not.toBeNull();
    expect(recordsZone?.querySelector('[data-testid="confirmed-record-severity-breakdown"]')).not.toBeNull();
    expect(recordsZone?.textContent).toContain('Dock 紧凑备注摘要');
    expect(recordsZone?.textContent).toContain('1 批次');
    expect(recordsZone?.textContent).toContain('批注 1');
    expect(recordsZone?.textContent).toContain('测量 0');
    expect(document.querySelector('[data-testid="annotation-table-row-anno-dock-1"]')).not.toBeNull();
    expect(document.body.textContent).toContain('Dock 批注标题');

    mounted.unmount();
  });

  // 批量收起按钮留在 AnnotationWorkspace（见 AnnotationWorkspace.test.ts），
  // ReviewPanel 现在挂的是统一批注单，这里改为覆盖文字/云线批注同表列出。
  it('统一批注单同时列出文字批注与云线批注', async () => {
    toolStoreMock.annotations.value = [
      {
        id: 'anno-text-1',
        formId: 'FORM-001',
    ...
    expect(document.querySelector('[data-testid="annotation-table-row-anno-text-1"]')).not.toBeNull();
    expect(document.querySelector('[data-testid="annotation-table-row-anno-text-2"]')).not.toBeNull();
    expect(document.querySelector('[data-testid="annotation-table-row-cloud-1"]')).not.toBeNull();
    expect(document.querySelector('[data-testid="annotation-table-empty"]')).toBeNull();
    expect(document.body.textContent).toContain('文字批注 1');
    expect(document.body.textContent).toContain('文字批注 2');
    expect(document.body.textContent).toContain('云线批注');

    mounted.unmount();
  });

  it('后端有确认记录但批注列表为空时，确认记录区仍展示批次、批注单落到空态', async () => {
    sortedConfirmedRecords.value = [
      {
    ...
    const recordsZone = document.querySelector('[data-testid="review-workbench-confirmed-records-zone"]');
    expect(recordsZone).not.toBeNull();
    expect(recordsZone?.textContent).not.toContain('暂无确认记录');
    expect(recordsZone?.textContent).toContain('1 批次');
    expect(recordsZone?.textContent).toContain('批注 1');
    expect(document.querySelector('[data-testid="annotation-table-empty"]')).not.toBeNull();
    expect(document.body.textContent).toContain('当前任务下还没有可浏览的批注');
  ...
  it('opens previewable attachments in the dock and keeps downloads explicit', async () => {
    ...
    // 下载链接必须带上原始文件名，否则落盘只会是 att-<uuid>.pdf
    expect(reviewAttachmentPreviewMock.buildDownloadUrl).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'attachment-pdf', name: 'drawing.pdf' }),
    );
  ...
  it('lets the requester supplement and delete attachments at the sj node', async () => {
    reviewAttachmentDeleteMock.mockClear();
    reviewAttachmentDeleteMock.mockResolvedValue({ success: true });
    currentTask.value = createTask({
      currentNode: 'sj',
      ...
      attachments: [
        {
          id: 'attachment-1',
          name: 'note.pdf',
          url: '/files/review_attachments/note.pdf',
          mimeType: 'application/pdf',
          uploadedAt: 1710000001000,
        },
      ],
    ...
    expect(document.querySelector('[data-testid="review-attachment-supplement-zone"]')).not.toBeNull();
    const deleteButton = document.querySelector(
      '[data-testid="review-attachment-delete-attachment-1"]',
    ) as HTMLButtonElement | null;
    ...
    expect(reviewAttachmentDeleteMock).toHaveBeenCalledWith('attachment-1');
  ...
  it('keeps attachments read-only outside the sj node and for non-requesters', async () => {
    currentTask.value = createTask({
      currentNode: 'jd',
      requesterId: 'reviewer-1',
      attachments: [
    ...
    expect(document.querySelector('[data-testid="review-attachment-supplement-zone"]')).toBeNull();
    expect(document.querySelector('[data-testid="review-attachment-delete-attachment-1"]')).toBeNull();
    expect(document.body.textContent).toContain('当前节点只读');

    mounted.unmount();

    // sj 节点但当前用户不是发起人：同样只读
    currentTask.value = createTask({
      currentNode: 'sj',
      requesterId: 'designer-1',
      attachments: [
    ...
    const remounted = await mountReviewPanel();
    await settleVue();
