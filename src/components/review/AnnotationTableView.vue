...
function getRowElements(): HTMLElement[] {
  ...
  // 限定 tabindex 才能排除表头行与展开详情行——它们同为 role="row" 但不该被方向键停留
  return Array.from(rootEl.value.querySelectorAll<HTMLElement>(
    '[role="row"][tabindex="0"], [role="listitem"][tabindex="0"]',
  ));
}
...
function clearFilters() {
  // 先掐掉 debounce，否则 300ms 后旧关键词会被重新写回 filters
...
/** 有数据但被筛空 ≠ 根本没有批注，两者的空状态文案和出路完全不同 */
const isFilteredEmpty = computed(() => (
  totalCount.value === 0 && props.items.length > 0 && hasActiveFilters.value
));
...
function ariaSortFor(key: AnnotationTableSortKey): 'ascending' | 'descending' | 'none' {
  const state = sortIconState(key);
...
<template>
  <section ref="rootEl"
    ...
    <div v-if="totalCount === 0"
      ...
      :data-empty-reason="isFilteredEmpty ? 'filtered' : 'no-data'">
      <Inbox class="h-9 w-9 text-slate-300" />
      <template v-if="isFilteredEmpty">
        <div class="text-sm font-semibold text-slate-700">没有符合筛选条件的批注</div>
      ...
      <template v-else>
        ...
        <div class="max-w-md text-xs leading-5 text-slate-500">{{ emptyDescription }}</div>
      </template>
    </div>

    <!-- Table head + body · Wide / Medium 档 -->
    <template v-else>
      <!-- rowgroup / row / gridcell 必须由 grid 拥有，缺了这层容器整套表格语义对读屏软件不成立 -->
      <div v-if="!isCompact" role="grid" class="flex min-h-0 flex-1 flex-col">
        <!-- Head（Compact 不显示表头）-->
        <div role="row" class="flex h-9 items-center border-b border-slate-200 bg-slate-50 px-4 text-[11px] font-semibold text-slate-950">
          ...
          <div role="columnheader" class="w-24 text-center">操作</div>
        </div>
        ...
        <div role="rowgroup" class="flex-1 overflow-y-auto">
          <template v-for="(item, localIdx) in pagedItems"
            ...
            <div role="row"
              ...
              <!-- 序号 -->
              <div role="gridcell" class="w-10 text-center text-sm font-semibold text-slate-950">
                {{ (currentPage - 1) * pageSize + localIdx + 1 }}
              </div>

              <!-- 错误标记 -->
              <div role="gridcell" class="w-24">
              ...
              <!-- 处理情况 · Medium 下收窄 -->
              <div role="gridcell" class="pr-2 min-w-0" :class="isWide ? 'w-56' : 'w-40'">
      ...
      <div v-else role="list" class="flex-1 overflow-y-auto p-2 space-y-2 bg-slate-50/30"
        ...
        <article v-for="(item, localIdx) in pagedItems"
          ...
          role="listitem"
          tabindex="0"
          :aria-current="isActiveRow(item) ? 'true' : 'false'"
          :data-testid="`annotation-table-row-${item.id}`"
