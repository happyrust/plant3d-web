...
describe('MeasurementOverlayBar', () => {
  ...
  it('应在测量模式下显示底部操作条，并支持打开详情抽屉与删除当前测量', async () => {
    ...
    expect(host.querySelector('[data-testid="measurement-overlay-current-visibility"]')?.textContent).toContain('隐藏当前');
    expect(host.querySelector('[data-testid="measurement-overlay-all-visibility"]')?.textContent).toContain('全部显示');

    (host.querySelector('[data-testid="measurement-overlay-details-toggle"]') as HTMLButtonElement)?.click();
    await nextTick();
    expect(host.querySelector('[data-testid="measurement-overlay-drawer"]')).toBeTruthy();
