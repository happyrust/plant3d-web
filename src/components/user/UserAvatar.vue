...
<template>
  <div class="relative" @mouseleave="handleMouseLeave">
    ...
    <Transition enter-active-class="transition ease-out duration-100"
      ...
      <div v-if="showUserCard && !showDropdown && user"
      ...
      <div v-if="showDropdown"
        ...
        <div class="p-2">
          ...
          <div class="space-y-0.5">
            <button v-for="u in userStore.availableUsers.value"
              :key="u.id"
              ...
              <span v-if="u.id === user?.id"
                class="h-2 w-2 rounded-full bg-green-500" />
            </button>
          </div>
        </div>
        <div class="border-t p-2">
          <button class="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm text-gray-700 hover:bg-gray-100 transition-colors">
            <Settings class="h-4 w-4" />
            设置
          </button>
          <button class="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm text-red-600 hover:bg-red-50 transition-colors">
