...
<template>
  ...
  >
    ...
    <!-- File Selection Area -->
    <div @click.stop class="flex flex-none items-center gap-2 border-b border-gray-200 bg-gray-50 px-3 py-2">
      ...
      <select
        ...
        :disabled="store.availableFiles.value.length === 0 || store.isLoading.value"
      ...
      >
        ...
        <option
          v-for="file in store.availableFiles.value"
          :key="file"
          :value="file"
        >
          {{ file }}
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div v-if="tableData && tableData.rows.length > 0" class="flex-none max-h-48 overflow-auto border-b border-gray-200 bg-gray-50">
        <table class="w-full text-xs border-collapse">
          ...
          <tbody>
            <tr 
              v-for="(row, idx) in tableData.rows.slice(0, 100)" 
              :key="idx"
