...
import {
  buildPickRefnoCandidates,
  computeCloudLayout,
  createRectAnnotationRecordFromObb,
  buildCloudBillboardPolyline,
  isDtxInteractionReady,
  isSamePickAnchor,
  isSamePickCandidateSequence,
  nextPickCandidateIndex,
  resolveBoxPickRefnos,
...
describe('resolveBoxPickRefnos', () => {
  it('无 noun 过滤时原样返回框选结果', () => {
    expect(resolveBoxPickRefnos(['a', 'b'], [], () => null)).toEqual({
      kind: 'ok',
      refnos: ['a', 'b'],
    });
  });

  it('部分 noun 可解析时，只保留命中过滤的构件', () => {
    const findNoun = (refno: string) => (refno === 'bran_1' ? 'BRAN' : refno === 'valv_1' ? 'VALV' : null);

    expect(resolveBoxPickRefnos(['bran_1', 'valv_1', 'unknown_1'], ['BRAN'], findNoun)).toEqual({
      kind: 'ok',
      refnos: ['bran_1'],
    });
  });

  it('一个 noun 都解析不出来时报告类型缓存未就绪，而不是返回空集', () => {
    expect(resolveBoxPickRefnos(['a', 'b'], ['BRAN'], () => null)).toEqual({
      kind: 'noun_unavailable',
    });
  });

  it('框选本身为空时不算类型缓存未就绪', () => {
    expect(resolveBoxPickRefnos([], ['BRAN'], () => null)).toEqual({
      kind: 'ok',
      refnos: [],
    });
  });
...
describe('buildPickRefnoCandidates', () => {
  const passthrough = (refno: string) => refno;

  it('按距离由近到远排列光标射线上的重叠构件', () => {
    const candidates = buildPickRefnoCandidates(
      [
        { entityId: 'C', objectId: 'o:C:0', distance: 30 },
        { entityId: 'A', objectId: 'o:A:0', distance: 5 },
        { entityId: 'B', objectId: 'o:B:0', distance: 12 },
      ],
      [],
      passthrough,
    );

    expect(candidates.map((c) => c.refno)).toEqual(['A', 'B', 'C']);
  });

  it('同一 refno 的多次命中只保留最靠前的一次', () => {
    const candidates = buildPickRefnoCandidates(
      [
        { entityId: 'TUBI-2', objectId: 'o:TUBI-2:0', distance: 20 },
        { entityId: 'TUBI-1', objectId: 'o:TUBI-1:0', distance: 8 },
      ],
      ['BRAN'],
      () => 'BRAN-1',
    );
    ...
    expect(candidates).toEqual([
      { refno: 'BRAN-1', hitRefno: 'TUBI-1', distance: 8 },
    ]);
  });

  it('过滤不通过的命中直接丢弃，不占用轮换位', () => {
    const candidates = buildPickRefnoCandidates(
      [
        { entityId: 'STRUCT', objectId: 'o:STRUCT:0', distance: 3 },
        { entityId: 'PIPE', objectId: 'o:PIPE:0', distance: 9 },
      ],
      ['BRAN'],
      (refno) => (refno === 'PIPE' ? 'BRAN-1' : null),
    );

    expect(candidates.map((c) => c.refno)).toEqual(['BRAN-1']);
  });

  it('保留命中的子构件 refno，供目标本身无几何时兜底高亮', () => {
    const candidates = buildPickRefnoCandidates(
      [{ entityId: 'TUBI-9', objectId: 'o:TUBI-9:0', distance: 4 }],
      ['BRAN'],
      () => 'BRAN-9',
    );

    expect(candidates[0]).toEqual({ refno: 'BRAN-9', hitRefno: 'TUBI-9', distance: 4 });
  });
});

describe('nextPickCandidateIndex', () => {
  it('正向轮换到末尾后回到第一个', () => {
    expect(nextPickCandidateIndex(0, 3)).toBe(1);
    expect(nextPickCandidateIndex(2, 3)).toBe(0);
  });

  it('反向轮换（Shift+Tab）从第一个回到末尾', () => {
    expect(nextPickCandidateIndex(0, 3, -1)).toBe(2);
    expect(nextPickCandidateIndex(2, 3, -1)).toBe(1);
  });

  it('没有候选时固定返回 0', () => {
    expect(nextPickCandidateIndex(5, 0)).toBe(0);
  });
});

describe('isSamePickAnchor', () => {
  it('落在容差内视为原地重复点击', () => {
    expect(isSamePickAnchor({ x: 100, y: 100 }, { x: 103, y: 104 })).toBe(true);
  });

  it('超出容差视为新的一次拾取', () => {
    expect(isSamePickAnchor({ x: 100, y: 100 }, { x: 120, y: 100 })).toBe(false);
  });

  it('没有上一次锚点时不视为重复点击', () => {
    expect(isSamePickAnchor(null, { x: 0, y: 0 })).toBe(false);
  });
});

describe('isSamePickCandidateSequence', () => {
  const candidate = (refno: string, distance: number) => ({ refno, hitRefno: refno, distance });

  it('refno 顺序一致时认为仍是同一批候选', () => {
    expect(isSamePickCandidateSequence(
      [candidate('A', 1), candidate('B', 2)],
      [candidate('A', 1.01), candidate('B', 2.02)],
    )).toBe(true);
  });

  it('相机转动导致候选换人时不再轮换，而是重新拾取', () => {
    expect(isSamePickCandidateSequence(
      [candidate('A', 1), candidate('B', 2)],
