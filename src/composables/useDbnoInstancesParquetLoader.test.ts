...
function createSemanticSnapPointManifest(dbno: number) {
  const manifest = createManifest(dbno);
  return {
    ...manifest,
    tables: {
      ...manifest.tables,
      semantic_snap_points: { file: `semantic_snap_points_${dbno}.parquet` },
    },
...
describe('useDbnoInstancesParquetLoader', () => {
  ...
  it('ZoneStream bucket index 优先于残留的 dbnum manifest', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/files/output/parquet/manifest_7997.json')) {
        return new Response(JSON.stringify(createManifest(7997)), { status: 200 });
      }
      if (url.endsWith('/files/output/parquet/manifest_7997_buckets.json')) {
        return new Response(JSON.stringify({
          version: 1,
          ...
          buckets: [
            {
              ...
              zone_refno: '24381_1',
              tables: {
                transforms: { file: 'generation-1/zones/24381_1/shared/transforms.parquet', rows: 2 },
                aabb: { file: 'generation-1/zones/24381_1/shared/aabb.parquet', rows: 2 },
              },
            },
            {
              name: 'unit-24381_2',
              ...
              noun: 'BRAN',
              tables: {
                instances: { file: 'generation-1/zones/24381_1/units/24381_2/instances.parquet', rows: 3 },
                geo_instances: { file: 'generation-1/zones/24381_1/units/24381_2/geo_instances.parquet', rows: 2 },
              },
            },
          ],
        }), { status: 200 });
      ...
      if (String(init?.method || 'GET').toUpperCase() === 'HEAD'
        && url.includes('/files/output/parquet/generation-1/')) {
  ...
  it('从 semantic_snap_points.parquet 读取 PLINE 设计世界端点', async () => {
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/files/output/instances/manifest_7997.json')) {
        return new Response(JSON.stringify(createSemanticSnapPointManifest(7997)), { status: 200 });
      }
      throw new Error(`unexpected fetch: ${url}`);
    }));
    queryMock.mockResolvedValue({
      toArray: () => [{
        point_id: 'semantic:pline:24381_145714:TOS:start',
        instance_refno_str: '24381_145714',
        point_index: 0,
        kind: 'pline_start',
        name: 'TOS',
        x: 11,
    ...
    const { useDbnoInstancesParquetLoader } = await import('./useDbnoInstancesParquetLoader');
    const out = await useDbnoInstancesParquetLoader()
      .querySemanticSnapPointsByRefnoFromParquet(7997, '24381_145714');

    expect(out).toHaveLength(1);
    expect(out[0]).toMatchObject({
      id: 'semantic:pline:24381_145714:TOS:start',
      kind: 'pline_start',
      label: 'PLINE TOS 起点',
      world: [11, 22, 33],
    });
    expect(queryMock.mock.calls[0]?.[0]).toContain('instance_refno_str = \'24381_145714\'');
  });
