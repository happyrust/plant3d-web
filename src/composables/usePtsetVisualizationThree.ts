...
export function usePtsetVisualizationThree(
  ...
  function appendEntry(refno: string, response: PtsetResponse, unitFactor: number, policy: string, targetUnit: string, displayUnit: string, precision: number, gm: Matrix4) {
    ...
    for (const point of response.ptset) {
      ...
      // 标记几何构建在点集原始空间（worldPt，可能是 mm），再由 group.matrix
      // (gm) 缩放进场景米制；期望的视觉尺寸按场景单位定，再除以 gm 缩放回
      // 点空间，保证 parquet（米）与后端 API（mm）两种数据源大小一致。
      const gmScale = new Vector3().setFromMatrixColumn(gm, 0).length() || 1;
      const pboreSceneSize = point.pbore * unitFactor * gmScale;
      const crossSize = Math.max(0.2, pboreSceneSize * 0.15 || 0.6) / gmScale;
      ...
      if (transformed.worldDir) {
        const arrowLen = Math.max(0.6, pboreSceneSize * 0.6 || 2.0) / gmScale;
