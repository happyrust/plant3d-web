...
export class DtxViewer {
  ...
  private _setupDefaultLights(): void {
    // E3D 风格读色优先：压低环境光避免整体发灰，主光从上方给出形体，
    // 冷色补光保持背光面的色相可辨（参数依据 docs 中 E3D 配色调研结论）。
    const ambient = new AmbientLight(0xffffff, 0.26);
    ambient.name = 'DtxAmbientLight';
    this.scene.add(ambient);

    const dir0 = new DirectionalLight(0xffffff, 1.15);
    dir0.position.set(0.7, 1.05, 1.5);
    dir0.name = 'DtxDirectionalLight0';
    this.scene.add(dir0);

    const dir1 = new DirectionalLight(0xd7e3f4, 0.5);
    dir1.position.set(-0.9, -0.55, 0.5);
