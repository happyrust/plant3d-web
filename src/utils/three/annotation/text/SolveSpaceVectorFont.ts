...
function makePwlArc(contour: GlyphContour, isReversed: boolean, cp: Vec2, radius: number, a1: number, a2: number): void {
...
function getGlyphBBox(glyph: Glyph): { minx: number; maxx: number; miny: number; maxy: number } {
...
function readFloatDecimal(s: string, i0: number): { value: number; next: number } {
  let i = i0
  const start = i
  const sign = s[i]
  if (sign === '+' || sign === '-') i++
  ...
  let hasDigit = false
  while (isDigit(s[i])) { i++; hasDigit = true }
  if (s[i] === '.') {
    i++
    while (isDigit(s[i])) { i++; hasDigit = true }
  }
  ...
  if (s[i] === 'e' || s[i] === 'E') {
    ...
    if (es === '+' || es === '-') j++
    let hasExp = false
    while (isDigit(s[j])) { j++; hasExp = true }
    if (hasExp) i = j
  }
...
function readHex16(s: string, i0: number): { value: number; next: number } {
  ...
  if (!Number.isFinite(value)) throw new Error(`Invalid hex16 '${hex}' at ${i0}`)
...
export class SolveSpaceVectorFont {
  ...
  static async loadFromGzipUrl(url = '/fonts/unicode.lff.gz'): Promise<SolveSpaceVectorFont> {
    const resp = await fetch(url)
    if (!resp.ok) throw new Error(`Failed to load SolveSpace font: ${url} (${resp.status})`)
    const ab = await resp.arrayBuffer()
    const text = await gunzipToText(new Uint8Array(ab))
  ...
  loadFromLffText(lffText: string): void {
    ...
    if (this.firstGlyphIndex < 0) throw new Error('Vector font contains no glyphs')
    ...
    for (const m of this.lffData.matchAll(re)) {
      ...
      if (name === 'letterspacing') {
        ...
        const space: Glyph = { contours: [], leftSideBearing: 0, boundingWidth: 0, advanceWidth: Number(value) }
  ...
  getCapHeight(forCapHeight: number): number { return forCapHeight }
  ...
  getExtents(forCapHeight: number, str: string): { x: number; y: number } {
    return { x: this.getWidth(forCapHeight, str), y: this.getHeight(forCapHeight) }
  }
  ...
  getGlyph(codepoint: number): Glyph {
    ...
    if (off === null) {
      if (codepoint === 0xfffd) {
        ...
        if (q) return q
        return { contours: [], leftSideBearing: 0, boundingWidth: 0, advanceWidth: 0 }
      }
  ...
  private parseGlyphAt(offset: number): Glyph {
    ...
    const glyph: Glyph = { contours: [], leftSideBearing: 0, boundingWidth: 0, advanceWidth: 0 }
    ...
    while (i < s.length) {
      ...
      if (ch === 'C') {
        ...
        for (const c0 of baseGlyph.contours) {
          glyph.contours.push({ points: c0.points.map((p) => ({ x: p.x, y: p.y })) })
        }
        continue
      }
      ...
      while (i < s.length) {
        const fx = readFloatDecimal(s, i); i = fx.next
        if (s[i] !== ',') throw new Error(`Expected ',' at ${i}`)
        i++
        const fy = readFloatDecimal(s, i); i = fy.next
        ...
        if (s[i] === ',') {
          ...
          i++
          const fb = readFloatDecimal(s, i); i = fb.next
    ...
    glyph.advanceWidth = glyph.leftSideBearing + glyph.boundingWidth + this.rightSideBearing
    return glyph
  }
}
...
export function getSolveSpaceBuiltinVectorFont(url = '/fonts/unicode.lff.gz'): Promise<SolveSpaceVectorFont> {
  if (!_builtinFontPromise) {
    _builtinFontPromise = SolveSpaceVectorFont.loadFromGzipUrl(url)
  }
...
async function gunzipToText(data: Uint8Array): Promise<string> {
  ...
  if (!DS) {
    throw new Error('DecompressionStream is not available; please run in a modern Chromium-based browser')
  }
