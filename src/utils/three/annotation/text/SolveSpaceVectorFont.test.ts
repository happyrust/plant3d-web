...
describe("SolveSpaceVectorFont", () => {
  ...
  it("supports bulge arcs (MakePwlBulge) and traces to line segments", () => {
    ...
    font.trace2D(10, 0, 0, "0", (ax, ay, bx, by) =>
  ...
  it("loadFromGzipUrl 应兼容“已自动解压”的纯文本 payload", async () => {
    const payload = new TextEncoder().encode(TEST_LFF);
    ...
    const font = await SolveSpaceVectorFont.loadFromGzipUrl(
  ...
  it("getSolveSpaceBuiltinVectorFont 首次失败后应允许重试恢复", async () => {
    const payload = new TextEncoder().encode(TEST_LFF);
    const attempts = new Map<string, number>();
    const url = "/fonts/retry-case.lff.gz";
    ...
    vi.spyOn(globalThis, "fetch" as any).mockImplementation(
      async (input: RequestInfo | URL) => {
        ...
        attempts.set(key, n);
        if (key.includes("retry-case") && n === 1) {
          throw new TypeError("Failed to fetch");
        }
        return new Response(payload, { status: 200 }) as any;
      },
    );

    await expect(getSolveSpaceBuiltinVectorFont(url)).rejects.toThrow();
    const font = await getSolveSpaceBuiltinVectorFont(url);
    expect(font.getWidth(10, "A")).toBeCloseTo(10, 6);
  });
});
