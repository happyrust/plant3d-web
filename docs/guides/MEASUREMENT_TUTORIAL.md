...
`位置/标高` 单击一次完成，结果包含未受 Viewer recenter 影响的工程 `World X/Y/Z`、绝对标高和相对标高。距离结果默认同时显示总长及带正负号的 `E/N/U` 轴向分量（E3D 工程坐标语义，可在样式设置中关闭）；它们统一跟随全局显示单位与精度设置，默认 `mm`、0 位小数（E3D 工程惯例，可改回 m/cm）。
