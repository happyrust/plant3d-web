...
- `距离测量：捕捉起点（P-Point / Item 原点）`
- `距离测量：捕捉终点（P-Point / Item 原点）；点空白取消当前点选`
...
- P-Point 从哪来跟数据源开关走：缺省的 gen-model 源按需向服务端请求（`POST /api/v1/element/ptset`，直读目录 P 点集，不依赖模型包）；`?model_source=legacy` 时仍读模型包的 `ptsets.parquet`，缺表再问 `:3100`。服务端不带这条接口时提示条会写明「未提供该构件的 P-Point」。

浮动条是 Object Snap Filter，只控制哪些点源可以捕捉；候选是否显示在测量面板中独立设置。`Item 原点`（Item Origin，元素原点）是模型实例变换定义的原点，不等同于 E3D Item 端点或几何中心。`模型表面点`属于自由近似点，默认不捕捉。
...
`位置/标高` 单击一次完成，结果包含未受 Viewer recenter 影响的工程 `World X/Y/Z`、绝对标高和相对标高。P0 距离结果按 `Distance / Offset X / Offset Y / Offset Z / Direction` 五行显示；在接入 E3D COORD/WRT 参考系前，轴标签明确使用 `World X/Y/Z`，不把固定的 `E/N/U` 映射冒充成已验证的 E3D 语义。显示值统一跟随全局单位与精度设置，默认 `mm`、0 位小数（可改为 m/cm）。
...
- `显示直接斜线尺寸`：对应 E3D `Show linear dimension`，只控制完成结果的直接尺寸线，不决定是否保存结果。
  开启时，单一正向轴向的测量（例如只量一段高度）与 E3D 一样只画直接线、不再叠画重合的轴向分量。
- `Perpendicular to`：对应 E3D `Perpendicular to`。开启后第二点若吸到带方向的 P-Point，测的是起点到该
  P-Point 轴线（无限直线）的垂距；吸到圆面关键点时测到该无限平面的垂距；普通表面点则退化为点到点。
  结果表改为 `Distance / Vertical / Horizontal / Direction`，参考系固定 World（与 E3D 一致，WRT 控件禁用），
  图形为起点到垂足的直接线加 Vertical / Horizontal 两条腿（任一腿短于 1mm 时不画）。起点已落在目标线 / 面上时
  提示 “Perpendicular distance is 0” 并回到起点选择。
- `测量后保留为标注`：Web 扩展；关闭时本次结果仍在结果面板和临时图形中，但不加入持久测量列表，可用“保留本次”补存。
