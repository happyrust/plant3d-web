...
> 状态：**已拍板**（2026-09-14 23:0x 用户拍板：§9 十项全部按「推荐」列定稿，决策 **d-565**；U0–U2 自此可开工，U0 的 `annotationScope.ts` / `useAnnotationDraftSession.ts` 纯函数与单测同日先落、不接 UI。排期与配套拍板见 `2026-09-14-review-next-steps-plan.md` §3 / §6）。
...
> **2026-09-14 23:0x 已拍板：十项全部按「推荐」列定稿（决策 d-565）。**「不建议」列保留为被否决方案的记录；要改任一项另起决策 supersede d-565 并同步本表。
