...
> 2026-09-15 11:3x 起草，用户在测量对齐方案 §7 Q7 上追加「选 (b)：先出一页方案」。**状态：待拍板，未动手。**
...
1. 零长（相交 / 重合）：不落记录 + 提示（建议）还是落一条 0 记录？
2. 面 × 面平行的起点：拾中点（建议）还是照 E3D 的 bug 用第一个顶点？
3. 入口：距离结果卡的 `Distance` 下拉（建议）还是浮条上的第三个子模式？
4. 批准后另起决策 supersede `d-616` 第 2 点；本方案 §2 表即那条决策的正文。
