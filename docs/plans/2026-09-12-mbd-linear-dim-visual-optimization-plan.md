...
- 验证：单测 59 文件 / 338 通过（`occlusionPolicy.test.ts` 4、theme +1、painter +1（三套缓冲 α 与 `transparent` 切换、
  ...
  **实机对照（BRAN 24381_145018，live gen-model `:18122`，三个相机）**：正面淡 `900.51`、背面淡 `1834.19` 与阀门位号
  ...
  （第三轮，现口径，只有真被别的几何挡住的阀门位号淡；独立射线复核 6/6 与内核一致）。ELBO 近景补验 9 弯头 × 14 方向：
  自己的 piece 挡在锚点前的 46 个视图里 26 个亮、20 个被别的 piece 挡住而淡，独立复核 124/124 一致。截图与读数见验证目录「实机对照」小节。
...
- 标签块 / 引线 / 方框（PRD T3）同日稍后落地（ADR 0058）：`label` + 配对 `leader_line` 呈现为屏幕定尺 billboard
  （坐标卡片 + 圆点 / 位号方框 / 弯头药丸），三级 LOD（坐标卡片与位号常显、弯头药丸中景起、角度行 / 分支名 / 坡度 / skew 辅助
  仅近景），标签体按确定顺序让开尺寸数字与其它标签；`mbd_3d=0` 同时剥掉。Chrome 全类别三视角统计与截图见同一验证目录
  `tags-*.png`（远景 4 标签绘 / 10 隐、0 相压）。SVG 导出的三维文字 2026-09-13 起按文字平面的屏幕单应精确导出
  （`glyph-run.perspective` + `kernel/geometry/homography.ts`），实心箭头输出填充多边形，与视口一致（验证目录「SVG 导出对齐视口」段）。
- 标签体避让管件包围盒、三维尺寸线与坐标 gizmo 2026-09-13 落地（ADR 0059）：宿主按设计空间区域交出构件包围盒角点、内核投影成
  凸包，其它布局的描边是障碍线段，gizmo 命中区按容器坐标交出当障碍矩形；候选位四圈、先零侵入再最少加权侵入、不为躲障碍出屏。
  Chrome 三视角对照见同一验证目录 `tags-obstacles-*.png`（中景尾端卡片与位号离开阀体；近景阀门凸包盖住半个画布时取最少侵入位；
  远景位号方框让开 gizmo）。
