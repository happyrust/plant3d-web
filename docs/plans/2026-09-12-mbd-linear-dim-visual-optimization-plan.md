...
- 标签体避让管件包围盒与三维尺寸线 2026-09-13 落地（ADR 0059）：宿主按设计空间区域交出构件包围盒角点、内核投影成凸包，
  其它布局的描边是障碍线段；候选位四圈、先零侵入再最少加权侵入、不为躲障碍出屏。Chrome 三视角对照见同一验证目录
  `tags-obstacles-*.png`（中景尾端卡片与位号离开阀体；近景阀门凸包盖住半个画布时取最少侵入位）。
