...
### 9.12 叠层挪到后处理之后单独一遍画、屏幕文字对齐像素格（2026-09-14，fable-5-1-17；用户「文字的显示还要做的更清晰一些」→ 拍板「C + D 一起做」；ADR 0064）

- **起因**：1× DPR 截图里药丸文字中位亮度 179（目标 62）——1.8 px 笔画的实心核只有 0.4 px；生产上帧帧走 EffectComposer（`hasOutline()` 只看 helper 存在），叠层被 FXAA 抹糊、在线性光里混合、平色再过 ACES（白卡片 233、深色文字被压到 9）；直接路径又把线性分量原样写进 sRGB 画布，两条路径颜色不一。
- **做法**：`DimensionViewport.overlayScene` + `renderOverlay(renderer, camera)`（整帧之后、只清深度、同相机），`ViewerPanel` 三处调用；画家颜色写 sRGB 分量；屏幕字形段 `pixelSnap`：笔画取整到设备像素（最少 2 个）、横竖笔两端吸格；inspection α 0.92 / 0.80 → 0.65 / 0.35（sRGB 里混合，对比保留率 = α）。适配器 `getScene` 删除。
- **验证**：单测 59 文件 / 362 通过（+3）；eslint 0；type-check 改动文件 0 新增。实机 before（HEAD worktree :3102）/ after（工作树 :3101）同相机：1× 药丸文字亮度 < 100 的像素 2 → 234；2× 药丸 43 → 1062、卡片最暗 9 → 23（= 名义色）、卡片纯白、三维数字白边干净；inspection α 0.65 / 0.35 的对比保留率 22 % / 7 % → **61 % / 31 %**。读数与图见验证 README「叠层挪到后处理之后 + 屏幕文字对齐像素格」。决策 **d-520**。
- **仍留**：三维 framed 文字不吸格；~~尺寸线 1.2 px 在 1× 上仍无整像素核，要不要「最少 1 个设备像素整宽」另拍~~（§9.13）；2× 上文字重了一档，过重就 `textStrokeWidthPx` 1.8 → 1.6。

### 9.13 尺寸描边也取整到设备像素、最少 2 个（2026-09-14，fable-5-1-26；用户「给尺寸线也定「最少 1 个设备像素整宽」：先在 1× / 2× 实机量 1.2 px 尺寸线的实心核，再照 ADR 0064 的取整口径改并另起决策」；ADR 0064「补」）

- **先量**：BRAN `24383_67485` elbo-mid 相机、真实 Chrome（vite 起在 HEAD 的独立 worktree `:3103`，避开别的会话的 HMR），每像素覆盖率由绿通道对背景的下降反解——1× 上 1.2 px 尺寸线峰值覆盖率 0.85、只有 25 % 的横截面有满覆盖像素（核 0.2 px）；2× 2.4 设备像素、核 1.45 px、100 %。
- **做法**：`hintedLineStrokeWidthPx` = max(**2**, round(w · dpr)) / dpr，不带 tag 色调的尺寸描边（尺寸线 / 尺寸界线 / 引线 / 标记）走它并 `pixelSnap = 1`（轴对齐的线吸格）；tag 边框 / 引线与 framed 文字不动；命中区 / SVG 仍名义 1.2 px。下限 2 不取 1：下限 1 实测在 1× 上把核从 25 % 打到 4 %、峰值 0.76，更淡；下限 2 → 100 %、峰值 1.0。2× 两种下限同为 2.4 → 2 设备像素（核 1.0 px）。代价：1× / 1.25× 上线与文字同重（2 个设备像素）。
- **验证**：单测 59 文件 / 363 通过（+1）；eslint 0；type-check `src/dimension` 0 新增。实机 before / 下限 1 / 下限 2 的 1× · 2× 横截面统计与三联图见验证 README「尺寸描边取整到设备像素、最少 2 个」（`dimension-line-*`）。决策 **d-537**。
- **仍留**：~~tag 边框 1 px / 引线 0.9 px 没有实心核也不吸格，边框是轴对齐矩形、吸格收益最直接，要不要一并另拍~~（§9.14）；1× 线嫌粗只有下限 1（更淡）或名义 1.2（无核）两条路，另起决策 supersede d-539。

### 9.14 标签的边框 / 方框 / 引线也取整到设备像素、最少 2 个、直边吸格（2026-09-14，fable-5-1-26；用户「把 tag 边框（1 px）与 tag 引线（0.9 px）也纳进同一口径：取整到设备像素、最少 2 个、轴对齐的边框吸格，照这轮的法子先量再改」；ADR 0064「补二」）

- **先量**：同一相机（`24383_67485` elbo-mid，HEAD worktree `:3103`），描边色为参数、两侧背景分开估——1× 上位号方框 1.2 px 峰值 0.83 / **0 %** 横截面有实心核，且四条边卡在不同亚像素相位（顶 0.91 / 左 0.61）；卡片边框 1 px 0.72 / 1 %（顶边 0.51 对半分两行）；引线 0.9 px 0.70 / 0 %。2× 本就有核。
- **做法**：`scenePainter` 里除 framed 三维文字外的每一条屏幕空间描边都提示——字形段 `hintedTextStrokeWidthPx`、其余（尺寸描边 + `tag-border` / `tag-frame` / `tag-leader`）`hintedLineStrokeWidthPx`（下限 2）并 `pixelSnap = 1`，卡片 / 方框直边吸格、圆角弧段与斜引线不动；theme 名义值不改，命中区 / SVG 仍名义。1× 修后方框 / 卡片边框每横截面整 2 个满覆盖像素、引线核 1.03 px；2× 方框 2.4 → 2 吸格核 2.0、引线 1.8 → 2 核 1.11。代价：1× / 1.25× 方框深色边框重了一档、与文字同重。
- **验证**：单测 59 文件 / 363 通过；eslint 0；type-check `src/dimension` 0 新增。实机前后图与统计见验证 README「标签的边框 / 方框 / 引线也取整到设备像素」（`tag-stroke-*`）。决策 **d-539 取代 d-537**。
- **仍留**：framed 三维文字与白边不吸格；1× 上文字 / 尺寸线 / 边框 / 引线同为 2 个设备像素，层次只靠颜色分，要恢复「文字比线重」得改文字到 3 px（另拍）。
