...
- **「新增」场景真机（22:5x）**：EQUI 24384/24776 618→628（另一会话第 2 轮加的 BOX 24384/26495）→ 横幅「该构件在版本 A 不存在（新建于 A 之后）」、
  「变更 29 / 29」、`@618 exists:false`。顺手撞到同根的两个缺口：该 BOX 632 又被删、当前会话里已不在 → 树定位不到（「1 个变更未能定位」、树空）、
  右侧属性面板 404 红条（§5.2 只兜 `deleted`）。~~候选修法见 README §5.3，未动、等拍板。~~ **2026-09-19 10:1x 两条都已修，见下条。**
- **顺手**：`ModelTreePanel.applyTreeDiffContext` 里既有的 3 条 TS 报错（可选字段写成 `字段: undefined` 让类型谓词不成立）改成条件展开，`vue-tsc` 该文件归零。
- **B 版之后又被删的构件（2026-09-19 10:1x，用户拍板修上面那两个缺口）**：`resolvePaths` 改成「每条变更一份解析计划」——非删除的先试自己，
  查不到再沿 `ownerRefno` 链逐级试幽灵挂载点（带 `expandSelf`）；「自己查不到」即判定它当前会话里已经没有了，记进 `unresolved`，
  `buildResult` 里和被删构件走同一条幽灵分支，徽章按 `model.status` 给、行尾标「当前已不在」（`ModelTreeRow`）。解析没落定前仍记「未定位」，
  不抢着渲染。右侧属性面板那半边只差首条选中的时机：`treeDiff.apply` 返回解析 promise，`applyTreeDiffContext` 对「不是删除、当前树里又还没有」
...
- **面板**（`ModelUnitVersionComparePanel.vue`，标题改「节点版本」）：受理任意 refno；**对比范围**开关 `仅自身 / 所有子节点`
  ...
  **2026-09-21 19:4x 已做**：不止一组时一颗总按钮把变了的单元一起进三维，按份并发 ≤ 2、进度「正在生成历史投影 n / m」、一发 `open` 带 `units[]`，
  收口计划 `2026-09-21-node-version-view-design-sync-and-gap-closure-plan.md` P2-a），
  ~~超阈值只提示不弹确认框（后端 `needs_confirm`，装载本就逐组）~~ **同日已改**：超过一次装载上限 20 个或服务端 `needs_confirm` 时先弹确认
  （全部生成 / 先装变化最大的 20 个 / 取消，P2-b）。纯函数在 `utils/nodeVersionTimeline.ts` / `utils/modelUnitVersionCompare.ts`。
