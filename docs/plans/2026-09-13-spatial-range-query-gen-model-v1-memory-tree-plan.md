...
- 状态：**已实施（2026-09-13 21:10；Plannotator 第 2 轮 `approved` 10:38）**，§5 六项均按推荐项 (a)；
  **§7 第 1–6 步已核**（第 2–3 步 22:03 在真服务 `127.0.0.1:18122` 上过；第 4 步的 503 信封以单测钉住，线上因树一直 Ready 复现不了；
  ...
  - 提交清单：old-aios-core `8758023`（P0）；gen-model-refactor `12bf601e9`（P1 路由 + 模块 + 单测）、`69f732e6b`（spec §4.13）、
    ...
    `2578ca8`（「加载当前页 / 只加载未加载」改为只动当前页）、其记录 = 本提交
...
- 顺手看到的**既有口径**（基线 `328ca8b` 就有，不是本计划改的）：`加载当前页` 走 `loadResults({flyTo:true})` →
  ...
  `resolveBatchRefnos` 加 `pages?: 'current' | 'all'`，「加载当前页」与并排的「只加载未加载」都传 `pages:'current'`、只动
  `resultSet.items`（摘要行「当前页 N 项 / 未加载 M 项」数的就是这一页）；分组按钮「加载本库 / 加载本专业」、全部显示、隔离仍走缺省 `all`
  （它们旁边的计数是全量）。单测 store +1 / drawer 断言同步。真机同一组 5 m / 每页 20 → 1387 项：「加载当前页」13 s 收工、
  「已加载 1 → 20，未加载 19 → 0」、场景对象 240 → 259；第 2 页「只加载未加载」27 s、「已加载 0 → 20」、场景 259 → 279；无错误横幅。
