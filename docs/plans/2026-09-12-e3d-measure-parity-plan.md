...
- **切片 5 ✓（2026-09-14 20:02，Web `3d6b16d`，决策 `d-529`）**：§2 #2 残余的 GENSEC 当 wrt 两条特例一起落地——`gphdimension.offsetType()` 826–856 的
  ...
  夹具化实机（本库无 GENSEC）+ 真 p-line 对 14 根 SCTN 的解法核对，golden MD §26。余：真 GENSEC 实机、~~gen-model-v1 下元素 wrt 帧的 transform 来源（另立项）~~（切片 6）。
