...
| 6 | Perpendicular to：点→线 / 面 / 点退化、World 帧、零距离告警 | ◐ | `perpendicularDistance.ts` + `perpendicularTargetProvider.ts`，G4 全部 ✓。目标 provider 按 E3D `GMFARC.perpendicularToPoint` 的分支序（`getLine()` → `getPlane()` → 点）接了：P-Point 轴、PLINE 线（实机 ✓，golden MD §17）、Graphics 边（`direction` + `segment`，标签用候选自己的名字）与 Graphics facet 面（`facet-plane`）、圆面关键点、Element 的 P1 → P2 `line()` 操作数（golden MD §16）。**Graphics 边 / 面当垂距目标的实机走查已补（2026-09-14，无代码改动，golden MD §23）**：先在 Perpendicular 关着时用 `Graphics × Cursor` 在同一条边 / 同一个面上采控制点（末点回代残差 0.0000 mm）独立定出那条无限线 / 无限面，再开 Perpendicular 量——垂足到基准线 / 面 0.0000 mm，`(S−F)·边方向 = 3.6e-16`、与面法向的 `|sin| = 1.1e-13`，距离与解析值 Δ 0.0000 mm。**Aid 类目标 ✓（2026-09-14 23:36，决策 `d-561`，golden MD §28）**：会话设计辅助线 / 面（Q3 拍板做最小实现）当 `getLine()` / `getPlane()` 的 DESIGNAID 分支——辅助线 → 无限线、辅助面 → 无限面，目标名就是辅助自己的名字；实机垂足与独立投影 Δ < 1e-6 m。**零距离告警 / 点退化 实机已补（2026-09-16 17:44，无代码改动，golden MD §36）**：P-Point → 自己的轴线 / TUBING 轴线、Graphics 面 → 同一张面（顶面精确共面、侧面同 / 异三角形）六次都走到零距离分支（草稿清空、回第 1 步、不落记录，与 E3D「尺寸重置 + 回 start」同口径）；点退化 P-Point → Screen 表面点 `572mm 近似 / 569 / 57 / W 84.2643 D`（Horizontal = 管外半径 57.15，Direction 垂足 → 源点与 E3D from → to 同向）；A → B 轴线那 9.5e-8 m 不算零、出 `0mm / 0mm / 0mm / S 0.0103685 W`。**撞出两处待拍板**：~~(a) 告警文案写进 `pickPointMessage` 后被同一处理函数里的 `clearHoverPtset()` 立刻置空，且 `pickPointMessage` 在 UI 上没有消费者——用户看不到任何告警（E3D 是模态 `alert.warning`）~~ **(a) 已修 `896fbbc`（2026-09-16 18:0x 拍板）**：先收辅助态再 `raiseMeasurementAlert`（`pickPointMessage` + warning toast），ViewerPanel 提示条第二行改放 `pickPointMessage`，单测 +1，`:8022` 重跑零距离两组六次告警可见（golden MD §36「(1) 已修」）；~~(b) 近零阈值 `1e-9 m` 对设计数据 1e-7 m 量级的噪声不生效（E3D `eq 0` 是精确零，显示什么未采）~~ **(b) 已修 `14b0d4b`（18:2x 拍板）**：阈值提到 `1e-6 m`（`PERPENDICULAR_ZERO_DISTANCE_M`），A → B 轴线重跑改成零距离告警，对照 B → A 轴线 1.1e-6 m 恰在阈值之上仍出结果；同一笔把 2,870 / 2,874 / Unable to convert / 角度造不出弧 也接上 toast。余 G7-02 的 E3D 运行时 golden 仍未采 |
| 7 | 三点角 Angle / Direction1 / Direction2 / 拒绝 0°·180° | ✓ | **内核已接线（2026-09-14，Web `d2e7c02`，Phase C 切片 1，决策 `d-486`；`75d15d4` 起与两线夹角共用网格噪声吸整 `angleSnap.ts`——臂分量 < 1e-6 归零、角度 1e-5° 吸整，golden MD §30「补采」）**：`threePointAngle.ts`（G6-01～03 golden）接进 `buildAngleMeasurementResultRows`，测量结果卡在角度模式出 `Angle / Direction1 / Direction2` 三行——角度取内核 minor 角、两条臂的单位方向按当前 wrt 帧表达、小数位缺省 2（E3D `Decimal Places` 缺省）；第三击落记录前先过内核，0° / 180° / 重合点**不落记录**、丢草稿回第 1 步、提示条给 E3D `alert.error` 那句话的等价文案。实机 `Angle 40.54°` 与三点解析角差 0.001436°、两条 Direction 逐位差 < 2e-4，退化三击拒收并回到第 1 步（golden MD §22）。~~**已知偏离**：方向仍是分量串（`X +0.3635 · …`）不是 E3D 罗盘串（`W 11.7755 N 66.8266 D`）——与距离结果表 `Direction` 行同一条既有偏离，要改两处一起改~~ → **2026-09-14 17:26 两处一起改成罗盘串**（Web `16e9a48` + `0e5c674`，决策 `d-515`，golden MD §25）：两条 Direction 走 `gphanglemeasure` 374–385 那条路（`.string()` 拆 token、数字按 Decimal Places 过 `!!realFmt` 留尾零、`.before('WRT')` 切尾巴），实机 Decimal Places 2 / 4 / 0 三档对账 |
...
| 9 | 两线夹角（LINEANGLE） | ✓ | **已落地（2026-09-15 05:33，Phase C 切片 4，决策 `d-587`，golden MD §30）**：纯内核 `src/measurement/kernel/lineAngle.ts` = `gmfArc.radius2Lines`（弧心 = 两线交点 / 异面取第一条线上最近点 / 线穿过面的点；线在面内 0° 弧、垂直于面 90°；半径 892–903 规则；平行 / 线平行于面拒收），入口是结果卡 `Angle` 下拉 `Angle 3 Points / Angle 2 Lines`（E3D `design.uic` 3538 `buttonMeasureAngleLines`，产品 UI 可达），两击 Graphics 边 / 边或面各转线 / 面操作数，结果表同一张四行 + `两线夹角` 一行。实机六组：异面 89.9996°、相交 90°（弧心 = 共顶点、半径 = 短边一半）、竖直边 ⟂ 顶面 90°、边在顶面内 0°、平行边拒收、边 ∥ 面拒收。余：E3D 运行时 golden（G6-04）未采；~~斜交的线 × 面只有单测~~（✓ 2026-09-15 10:28 / 10:32 补采：LOOP3 平面内 64.54° 的短梁棱 × 邻梁侧面、仪表支架 ORI 倾斜 70° 的盒体棱 × 水平盒体侧面，设计侧 64.540031 / 70.000000 对上；本库没有斜撑；~~管件没走~~ 2026-09-15 23:57 / 23:58 管件路补齐：1WCC0073 BRAN `24383/66662` 里 30° 下倾斜管的 TUBING 轴线 × ELBO 端面 59.99962° / × VALV 水平面 30.00038°（第一击本来就不必是直棱；第二击要先正对面片拉近，否则 12 px 内的边整包压过面）；golden MD §30「补采」；补采撞出的网格噪声在 `87b8970` 吸掉——出记录前臂分量 < 1e-6 归零、角度按 1e-5° 吸整，`70 Degrees` 不再配 `69° 59' 59''`）；~~第二击的 P-Point / DPOINT 当面只有 Graphics 面实机过~~（✓ 2026-09-16 10:15 / 10:18，`03f4458`：第二击的带方向点按 E3D `EDGPOSITIONDATA.getPlane()` 的 PPOINT / DPOINT 分支当「过该点、Z 沿其方向」的面（此前走 Intersect 的 POINTVECTOR 线），第一击仍是它的轴线；实机同一根斜管轴线 × VALV P100（U）30.00042° / × ELBO P2（N）59.99958° / × ELBO P1（沿线）89.99958°，MDS special 立柱棱 × 设计点 #1 90°——P-Point 面给出的正是设计侧值，Graphics 面那两行差的 4e-5° 是网格法向噪声；golden MD §30「补采（10:15 / 10:18）」） |
| 10 | Shortest（graphics × graphics） | ✓ | **按 E3D 实际行为收口（2026-09-15 11:24，决策 `d-616`，golden MD §32 静态 + §33 Web 实机）**：Web 的对应物 = 距离测量 × Graphics 过滤器两击，两击的 Distance = \|两拾中点\|，拾中点 = 边线上离射线最近点 / 射线 ∩ 面（与 E3D `positionData()` 差 ≤ 1e-7 m）；两平行边相距 8 mm 时 E3D / Web 都报 593 mm（点点）而不是 8。E3D「填进偏移字段」那层在 Web 无入口，N/A；不建 `shortestDistance.ts`。~~现有 clearance / 最近点是采样近似（上一计划 §3.2），不是 `gmfLine.shortest` 语义；G5 未采。~~ **静态前提清单（golden MD §32）**：E3D 3.1 产品里的 Measure Shortest = Picking Control 偏移字段右键填值，两次 Graphics 拾取后永远是**两拾中点的距离**（线 / 面分支不可达，无结果表 / witness）。按 E3D 实际行为，Web 的对应物 = 距离测量 × Graphics 过滤器两击（§12 已有），差的只是「填进偏移字段」这层（Web 无此字段）；按 `gmfLine.shortest` 设计意图做真最短距离则是 Web 增强、无 golden 可对。~~**待拍板 §7 Q7**~~ Q7 选 (a) |
...
| 14 | 拾取过滤器：Graphics（facet 边 / 面）、Screen、Aid、External | ✓（Graphics / Screen / Aid）· External 占位待拍板 | **Graphics ✓（2026-09-13）**：`mesh_graphics` 点源从已加载网格派生绘制边（线）/ 面（平面），只在 Graphics 过滤器下参与；实机 `面 → 边` 距离走通（golden MD §12）。Screen ✓（= `mesh_pick_point`）。**Aid ✓（2026-09-14 23:36，决策 `d-561`，golden MD §28）**：最小会话级设计辅助系统——`src/measurement/aids/designAid.ts`（GPHLINE / GPHPLANE 等价物：有限线、位置 + 法向的矩形面，缺省 5000 × 5000 mm）+ `useMeasurementAidStore`（会话内存，按创建顺序编号）+ 面板「设计辅助（Aid）」（两点 / 位置 + 方向 + 长度建线，位置 + 法向 / 过三点建面，填入最近一条测量的点，显隐删除）+ `design_aid` 点源（线：控制点 = 线上离射线最近处、带整条 `segment`；面：射线 ∩ 面且落在矩形上、带 `plane`）；只在 Aid 过滤器下参与（E3D `stdAid`），图形画在场景里。实机 Aid × Snap 取近端 Δ 0.0002 mm、面 射线 ∩ 面 离面 5e-8 m、Perpendicular to 线 / 面垂足 Δ < 1e-6 m。不做：POSITION / ARC / grid、颜色线型、持久化。External 仍占位灰掉。**2026-09-16 盘点**：Graphics 边 / 面自 §12 起又在 §23（Perpendicular to 以 Graphics 边 / 面为无限线 / 面）、§30（Angle 2 Lines 边 × 边 / 边 × 面）、§33（Shortest 两击 = 两拾中点）、§34（Web 增强 Shortest 的线 / 面 witness）实机反复用到，这一行 ◐ 只剩 External 一项——按 §3 它在 P2「可选 / 需拍板」，状态改记 ✓（Graphics / Screen / Aid），External 单列待拍板 |
| 15 | TUBING 轴线点、DPOINT | ◐ | **TUBING ✓（2026-09-13）**：`tubing_axis` 点源从直管对象的局部包围盒 × 放置矩阵派生管身轴线，两端吸到邻接 P-Point（`src/measurement/tubing/tubingAxis.ts`），按 E3D `EDGTUBING` 走线候选（Snap 近端 / Cursor 交点 / Mid-Point 等沿线 / Intersect 转 LINE），Any / Element 放行；实机 `轴线 → 轴线` Snap 与 Mid-Point 两条距离走通、校正端点与 E3D ELBO 145031 P1 差 0.001 mm（golden MD §14）。ATTA 按 E3D `line()` 跳过：gen-model 本就不在非 SPKBRK 的 ATTA 处断管，SPKBRK 断开的两段前端合并（`mergeTubingAxisAcrossPassThrough`，golden MD §15）。**DPOINT ✓（2026-09-14 23:40，决策 `d-563`，golden MD §29）**：纯内核 `src/measurement/dpoint/designPoints.ts`（`NUMB / POS / ORI` → `DPPS` = 属主 `world_transform` × POS、`DPDI` = ORI 的 Z 轴，`Rz · Ry · Rx`）+ 加载器 `designPointLoader.ts`（悬停叶子沿属主链到 ZONE 之下逐 host 查 `DPSE → DPCA / DPCY`，按 host 缓存 + nounCache）+ `design_point` 点源（随 P-Point 在 Any / Ppoint 下放行，优先级排在 P-Point 之后；Distance 沿 `DPDI` 偏移、Intersect 当点向量线、Perpendicular to 取「过 `DPPS`、Z is `DPDI`」的面——E3D `getLine()` 无 DPOINT 分支）。实机 MDS special STRU 23714/1111 的 DPCA #1：Snap 起点 = `(0, 0, −86)` Δ 8e-17 m、Distance 500 = `(0, 0, 414)`、Perpendicular 垂足投到 `Z is U` 的面 Δ 8e-17 m。`static_expectation`：本库唯一样本 `ORI (0, 0, 0)` 检验不出旋转分支。**非直管实机已补（2026-09-16 18:5x，无代码改动，golden MD §37）**：斜管四根（DN350 45° / 66°、DN20 13° / 50°，两端是 OLET / ELBO / BEND）Snap 两端全部落在邻接构件 P-Point 上 Δ 1e-14 m，Mid-Point / Perpendicular 垂足同级——倾角对派生无影响；**弯管（ELBO / BEND）作为拾取对象与 E3D 有差**：E3D `EDGELBOW / EDGBEND` 只有 `arc()`（中心线圆弧），Perpendicular 走 `getPlane()` 的弧面、Intersect 给 ARC 操作数，Web 的 ELBO / BEND 没有 `arc`——Element × Snap 拾不到、Element × Cursor 退化成表面点（实机 456 vs E3D 76 mm、1271 vs 7 mm）。~~要补是给 ELBO / BEND 一个 fillet 弧（P1 / P2 / POS 三点，不依赖 RADI）接进现成的 `circle-plane` provider——**待拍板**~~ **弯管 Perpendicular ✓（2026-09-16 19:04 拍板，`c4f20bd`）**：新 kernel `src/measurement/kernel/elementArc.ts`（fillet：两腿 `POS` → P1 / P2，R = T · tan(α / 2)，弧心在角平分线上，法向 u1 × u2；角点 = 点集 `world_transform` 平移列，`usePtsetSnap.getOrigin`），`elementArc` 挂在元素候选上、无候选时 `elementPickAsOperand` 在 Perpendicular 下给弧，喂 `resolvePerpendicularTarget` 的 `arc` → `circle-plane`；实机重跑 ELBO `76mm`（垂足 Δ 2.7e-15 m）/ BEND `7mm`（Δ 4.0e-15 m），Element × Snap 与 × Cursor 都成（golden MD §37「修后」行）。**弯管 Intersect ✓（2026-09-16 23:23，`9bbb501`）**：同一条弧当 ARC 操作数接进求交（内核 `intersectArcWith` + 弧 × 线 / 面 / 弧，弧永远是被求交的主体，取离「拾中这条弧的射线 ∩ 弧面」最近的交点；没有交点 = E3D `alert.warning('No intersection between picked items')`，只丢这一击），实机 ELBO 145121 的弧 × 两条腿上的隐含管轴线（相切）出的交点与独立算的切点差 5.5e-15 / 4.6e-15 m、两点合成 `449mm`，长管不相交出 warning 且弧留在会话里（golden MD §37 (1-b)）。**RTOR / CTOR 的环面中心圆 ◐**：`arc(dbRef)` = `through3Points(P1, P3, P2)`（不要 POS / RINS / ROUT）已接进 Intersect（`elementArc.ts` 环面分支 + 单测，本库无样本、未实机）；Perpendicular 那一路 E3D 走 `arc(item, refPosition)`（圆随拾中点沿轴挪 / 换 RINS / ROUT / 改截面圆），要目录属性，**待拍板**（golden MD §37 (1-c)）。交点记录不再标「近似」（`sourceInfo.exact` + 距离记录写 `provenance`，实机复核见 §37 (1-b)）。**2026-09-17 跑着的 E3D 实测更正（golden MD §39）**：① 上面「弯管 Perpendicular ✓」的前提不成立——`GMFARC.perpendicularToPoint` 只问 `getLine()` / `getPlane()`，`EDGELBOW` / `EDGCTORUS` 两张对象都只定义了 `arc()`，E3D 实际给的是**点到点**（同一枚弯头实测 551.094 mm、垂足 = 拾中点），② ARC 操作数这条路对，但**圆不一样**：`edgelbow.pmlobj` 58 取目录 `parameter[2]`，而这枚 ASME B16.9 弯头的 `parameter = [350, 356, 533]`——`[2]` 是外径、真正的弯曲半径在 `[3]`，E3D 的弧 R 356 切点离 P1 / P2 各 82.285 mm，Web 的 R 533 正好切在 P1 / P2；③ 环面 Perpendicular 那条 ◐ **收口为不用补**：E3D 也是点到点，Web 现在的退化行为就是对的。**用户 2026-09-17 拍板「两条都对齐 E3D」，`dcac130` 已实现**：Perpendicular 回退成点到点（弧只留给 Intersect，`ELEMENT_ARC_LABEL` 整条删掉）；弧半径改读 E3D 那一格（新内核 `elementArcRadius.ts` + 悬停异步取目录属性 `elementArcRadiusCache.ts`，BEND 走 `RADI`、ELBO 走 `SPRE → CATR → PARA[2]`，取不到就没有弧——与 E3D `(2,888)` 一致）。单测 185 文件 1647 用例全过；**真 UI 实机未做**（`:8022` 现在这个 build 没有 `element/ptset`，模型加载不了），补跑清单见 golden MD §39.6 |
| 16 | 拾取类型：Snap / Exact | ✓ | 拾取层 `pickType` Snap / Cursor；点候选、线候选（`GMFLINE` 近端 / 控制点）、面候选（射线 ∩ 平面）三类几何均接内核 |
| 17 | 拾取类型：Distance / Mid-Point / Fraction / Proportion / Intersect | ✓（Web 实机）· G8 E3D 对账待采 | 全部已接：Distance / Mid-Point / Fraction / Proportion 走 `pickDerivation.ts`（PLINE 线、Graphics 边、P-Point Distance 偏移）；Intersect 走 `intersectPickSession.ts` 两 / 三次子拾取（线 × 线 / 线 × 面 / 面 × 面 × 第三项，E3D 2,870 / 2,874 分型），实机 `ELBO 边 × VALV 边` 出交点（golden MD §13）；~~**◐ 只因 G8 运行时 golden 未采**~~ **2026-09-16 01:40 按 G8 同一组取值在 gen-model-v1 真模型上实机走通（golden MD §35）**：SCTN `24381/177301` 的 `PLINE NA`（5757.258 mm）上 `Distance[100]` 两端各进 100 → 5557.258、`Fraction[3]` → L/3 = 1919.086、`Proportion[0.25]` → 0.5 L = 2878.629、`Mid-Point` × 横梁中点 2981.570，派生点与 `element/plines` 手算差 7e-15 m，第二击都验到 `reverseSense`（近端换成 PLEND）；Intersect **面 × 面 × 面**（此前只有单测）在 BOX `24381/102278` 两个盒角各三次子拾取出 `交点`，与设计盒角差 1.8e-7 m、对角 114.978 mm，`Intersection[1] → [2] → [3]` 提示逐步对上。状态改 ✓：Web 已按 E3D 口径落地并实机；E3D 侧 G8 位置字串仍待对账（E3D 不在跑），与 #9 / #10 的处理一致。**2026-09-16 08:30 补采 §35 残余三支**（golden MD §35「补采」）：Distance 作用在 P-Point 上沿其方向偏移（ELBO P-Point 200 mm，斜方向那组竖向 ±0.998 mm 严格跟方向）、Proportion / Fraction 控制点落在线段外回近端（TUBING 轴线，仰 60° 让射线与轴线的最近点落到 B 之外 s = 1.043 → 回 B）、Intersect 面 × 面 × 线只与**第一**面求交（棱躺在 / 平行于第二面仍出角点）+ 线 ∥ 第一面 `(2,874)` 整包重置；剩 Fraction 同距取段起点（真指针像素到不了恰好相等，天然单测）与 G8 E3D 对账。撞出三处待拍板偏离记在 §35「已知偏离」：~~Any 下「模型表面点」盖过 TUBING 轴线~~（**已修 `737a02c`**：Any × surface 只在 Cursor 放行，与 Element 同口径；Any × Intersect 把表面点当元素拾取；尺寸 SnapPort 不受影响；Any 重跑第 1 个机位即拾到轴线）、~~P-Point 进场景优先 float32 的 DTX 矩阵（18 m 处 ~1 µm）~~、~~罗盘零容差 1e-10 放过设计数据里 1e-7 的横向噪声（`N 2.71502 W 90 U`）~~（**两条都已修 `2dc5ad1`**：`pickPtsetWorldTransform` 接口 float64 矩阵优先、DTX 回落，吸附与十字同一取法；`formatCompassDirection` 归一后 < 1e-6 分量吸整同 §30 口径；重跑 P-Point Distance Δ 1.9e-13～3.6e-15、纯竖直 `U`）。三处偏离清零 |
| 18 | Significant Snaps 开关 | ◐ | 拾取层 `significantSnaps`（覆盖条开关 + 提示尾巴 `Snap`），线候选带 `intermediates` 时按段派生；无 E3D 实机对照 |
| 19 | 提示文案结构 `<命令> <步> (<拾取类型>) <过滤器> :` | ✓（结构 + 矩阵已对照；D1 已拍板并改 `0c0d9eb`，改后实机四态截图入 golden MD §30；**E3D 运行时对账 §2–§5 全实机**（golden MD §38 / §40–§40.8，09-16 23:3x → 09-17 11:3x），D1–D8 全结，只余 §6 告警原文） | `formatMeasurementPrompt`：`距离测量 · 第 2/2 步 选择终点 (Mid-Point) Snap : ELBO P-Point #1`（E3D `EDGSTATE.prompt()` 结构，过滤器不进提示与 E3D 一致）；~~文案矩阵（Phase E）未对照~~ **2026-09-16 矩阵已落 `docs/verification/e3d-measure-prompt-matrix.md`**（对本机 Administration 1.8 PMLLIB：`EDGSTATE.prompt()` = `<pickPacket.prompt> <pickType.prompt> (<pick.prompt>)[ WP][ Offset][ Snap]` + `applyToView` 的 ` :`）：Measure distance `start` / `end`、perpendicular distance `start` / `end`、angle `root of angle` / `first point` / `second point` 每一步、七种 token（`Snap` / `Distance[v]` / `Mid-Point` / `Fraction[v]` / `Proportion[v]` / `Intersection[n]` / `Cursor`）、八个过滤器不进提示、Significant Snaps 缺省开——逐条对上。差异：**D1 两线夹角**在 E3D 是 `stdGraphics` 拾取（`positioning = false`、EDGPICK 无 prompt）→ `Measure angle between lines first line :` 不带 `(token)` 也不带 ` Snap`，~~Web 带（待拍板）~~ **2026-09-16 10:54 拍板去掉，`0c0d9eb`**：`formatMeasurementPrompt` 加 `positioning`（缺省 true），两线夹角两步传 false → `两线夹角 · 第 1/2 步 选择第一条线 : …`，单测 +2；D2 `Fraction[v]` ~~Web 取整、E3D 原样——定为不改（决策 `d-228`）~~ ~~2026-09-16 15:5x 用户改口、改掉（`e8ebc10`，决策 `d-269` 取代 `d-228`）：Web 落库不再归一、提示照 E3D 原样出输入值（`Fraction[2.5]`），内核仍 `int()`（2.5 → 2 段）~~ **2026-09-17 06:4x 实机翻案（golden MD §40）**：E3D 用户在 pad 输入框填 2.5 实际出 `Fraction[3]` / 回显 `3` / 落三分点（输入框 `!!integerFmt` dp 0 按格式回读），`Fraction[2.5]` 只有程序直写才出；**07:36 用户拍板 (a)、改掉（`67b127b`，决策 `d-342` 取代 `d-269`）**：Web 输入即按 dp 0 四舍五入，提示 / 回显 / 内核同一个整数，单测四处改口（token / V9 读回 0.4 → 0 / 输入框 2.5 → '3' / Graphics 立方体提示 `Fraction[3]` + 吸三等分点）；D3 步计数 / `（第一条：…）` 回显 / 冒号后目标名 / trailer 是 Web 加的（保留）；D4 ` WP` / ` Offset` 不做；D5 `(2,874)` 原文在 message file 未采，Web 文案同笔改「面 × 面 × 第三项没有唯一交点」（第三项是线时也说得通，单测 +1）；D6 E3D 另一条 `measureLineAngle`（只算角不画弧）2026-09-16 静态核清后**不做**（`2cd6602`：功能区 `Angle 2 Lines` 走的是画弧包，非弧包只服务三张设计表单的角度输入框；两条包角相等已由 `lineAngle.test.ts` 钉住）；D7 Intersect 面 × 面 × 弧在 E3D 是未处理的 PML 错误（09-17 实机 `(2,779)`，golden MD §39.2），Web 取舍拒收不消耗这一击（`9bbb501`）；**D8 Ppoint / Screen 下 E3D pad 整排拾取类型置灰**（`setPickGadgets` 199–201，09-17 11:3x 实机 §40.8）→ 09-17 15:5x 用户拍板浮条跟上（`82411f9`，决策 `d-379`；类型 / 提示 / 落库不动，只灰按钮 + 一句提示；真 UI 截图 `web-prompt-matrix-d8-pick-type-inert-live-*.png`）。运行时对账另见 Phase E：§2–§5 全部 `observation`，只余 §6 告警原文 |
...
- **P0（不补就不能说「E3D 有的我们都有」）**：~~#10 Shortest~~（✓ 2026-09-15，d-616，按 E3D 实际行为收口）、~~#9 两线夹角~~（✓ 2026-09-15，d-587）、~~#17 拾取类型~~（✓ 2026-09-16 Web 实机 golden MD §35；G8 E3D 对账待采）、~~#14 Graphics 边 / 面拾取（它同时解锁 Perpendicular 与 Shortest 的目标 provider）~~（✓ 2026-09-13 起，§12 / §23 / §30 / §33 / §34 实机反复用到；External 留 P2）、~~#13 在 gen-model-v1 下的 PLINE 供给~~（✓ 2026-09-14，d-394，golden MD §17 / §18）（#12 Element 显著点 2026-09-13 核对为非 E3D 口径，撤出 P0，d-336）。
- **P1（功能在、契约缺角）**：#4 Keep 生命周期（✓ 2026-09-14）、#5 Units 矩阵（✓ 2026-09-14，英制不做）、~~#7 角度内核接线 + Direction1/2~~（✓ 2026-09-14）、~~#8 角度单位 / 小数位~~（✓ 2026-09-14）、~~#19 提示结构~~（✓ 2026-09-16 矩阵 `docs/verification/e3d-measure-prompt-matrix.md`；D1 两线夹角 token / Snap 尾巴已拍板去掉 `0c0d9eb`）、#20 分层取消（✓ 2026-09-14，余右键 trace）。
...
- ADR 0060「测量拾取层对齐 E3D Positioning Control」已落（`docs/adr/0060-align-measurement-pick-layer-with-e3d-positioning-control.md`）：
  两维模型、Any = Element / Ppoint / Pline（+ 表面点——2026-09-16 `737a02c` 修订为只在 Cursor 类型放行，与 Element 同口径，ADR（1 修订））、Graphics 网格派生、Intersect 分型三条口径与被否决的替代。
...
- **2026-09-15 收口（§7 Q7 选 (a)，决策 `d-616`，golden MD §32 / §33）**：E3D 产品里的 Shortest 永远是两拾中点的点点距离，Web 距离测量 × Graphics 两击已经是它（§33 实机对上 E3D `positionData()` 口径）；下面的内核 / 入口设想对应的是 `gmfLine.shortest` 里不可达的分支，**本阶段不做**，#10 ✓。
...
- ~~入口：E3D 里 Shortest 挂在偏移菜单（G5-05 待采）。Web 放进距离模式的第三个子模式「最短距离」（两次 Graphics / Ppoint / Pline 拾取），结果表 `Distance / Vertical / Horizontal / Direction` + 两个 witness；辅助图形画 witness 线。~~（G5-05 静态已答：Picking Control 偏移字段右键；Web 不另开子模式）
- 与 clearance 的关系：Shortest 是**用户取两组几何的精确最短距离**；clearance 仍是构件对构件的服务端 nearest-points（上一计划 D1）。UI 文案区分，不互相冒充。

**golden gate**：~~G5-01～05 全部；Web vitest 与 E3D 逐组对账（容差 1e-3 mm）。~~ 改为：§32 静态 + §33 Web 实机（已过）；G5 运行时 golden 只作 E3D 侧对账补齐。

### Phase E · 交互细节与文案矩阵

- 提示文案矩阵（每模式 × 每步 × 拾取类型 × 过滤器）与 E3D 逐条对照，落 `docs/verification/e3d-measure-prompt-matrix.md`。**2026-09-16 已落**（对本机 Administration 1.8 PMLLIB 源，`static_expectation`）：结构与三条定位拾取命令每一步、七种 token、过滤器不进提示都对上；差异 ~~D1–D6~~ **D1–D8** 见该文 §7；D1（两线夹角不带 token / Snap）2026-09-16 10:54 拍板并改掉（`0c0d9eb`），改后的提示条 12:32 / 13:09 实机四态截图入 golden MD §30 补采，D5 的 Web 文案同笔改掉，D6（非弧的 `measureLineAngle`）静态核清后结掉——功能区那颗按钮走的是画弧包，非弧包是三张设计表单的取值入口，Web 不做，两者角相等由 `lineAngle.test.ts` 钉住，D2（`Fraction[v]`）~~定为不改（决策 `d-228`）~~ ~~2026-09-16 15:5x 用户改口、改掉（`e8ebc10`，决策 `d-269` 取代 `d-228`）——Web 提示照 E3D 原样出输入值，内核仍 `int()`~~ 2026-09-17 06:4x 实机翻案（E3D 用户路径填 2.5 出 `Fraction[3]` 落三分点，golden MD §40）、07:36 用户拍板 (a) 改掉（`67b127b`，决策 `d-342` 取代 `d-269`）——Web 输入即按 dp 0 四舍五入，提示 / 回显 / 内核同一个整数；D7（Intersect 面 × 面之后第三项是弧：E3D 是未处理的 PML 错误 `(2,779) Method <ARC>.INTERSECTION(<PLANE>,<PLANE>) not found`，09-17 实机 golden MD §39.2）Web 取舍拒收且不消耗这一击（`9bbb501`），E3D 那一路不采；**D8（09-17 15:5x 用户拍板、改掉 `82411f9`，决策 `d-379`）**：E3D pad 在 Ppoint / Screen 过滤器下把整排拾取类型按钮置灰（`setPickGadgets` 199–201，实机 §40.8），类型 / 提示不动——浮条跟上（`measurementPickTypeSelectable`，按钮 `disabled` + 一句「<过滤器> 下不起作用（E3D 同）」，单测 +1 与 D8 段，真 UI 截图 `web-prompt-matrix-d8-pick-type-inert-live-*.png`）；余 D4（不做）与 D5 的 E3D 原文对账。运行时对账：2026-09-16 23:3x 四条命令第 1 步（golden MD §38）、09-17 06:4x 七种 token 与两线夹角第 2 步（§40）、10:3x 距离 `(Snap)` 第 2 步 / 垂距第 2 步 / 三点角第 2、3 步（§40.6，带截图）、10:5x §5 三条尾巴 Snap 关 / WP / Offset 及组合（§40.7，pad 的 WP 按钮没工作平面时只弹 `No working plane has been defined.`）、11:3x §4 过滤器八串（§40.8，真点 pad 八颗按钮提示字字不变、亮灯与 `pickIndex` 一一对应）——**矩阵 §2–§5 已全部是 `observation`**（§2 只剩 D6 那条非弧包，不做），~~余 §4 过滤器 / §5 尾巴 / §6 告警原文~~ **只余 §6 告警原文（`P6-*`，含 D5 的 `(2,874)`）**。
...
- Shortest：~~G5 通过前 No-Go（沿用）。~~ 2026-09-15 按 E3D 实际行为收口（Q7 选 (a)，`d-616`）：静态 §32 + Web 实机 §33，#10 ✓；G5 运行时 golden 只作 E3D 侧对账补齐。
