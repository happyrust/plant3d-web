...
仅当 `target_face.kind ∈ {inner, outer, side, opening}`（主面；`opening` 09-18 加）：射线起点 = 源侧最近点 p，方向 = −n（指向墙面），`w_j.cast_local_ray_and_get_normal(&ray, max_toi = distance × 4 + 1, solid = false)`；命中 → `perpendicular = {distance_mm: toi, from: p, to: hit}`；未命中或 toi 与 distance 差 > max(1%, 0.05 mm) → `perpendicular = null` 并 warning `perpendicular_ray_missed`（最近点在棱 / 角上）。**09-18 补**：直打落空时把起点朝命中三角重心在面内挪 max(0.01, distance‰) mm 再打一次（面垂直于射线，toi 不变；`from` / `to` 仍按原起点算）——穿孔的管与洞壁同宽时最近点正落在洞壁与开孔面的共棱上，不挪就擦棱打空。**仍落空且最近点在共棱上**（q 处还有别的三角，`0dce7fee2`）：换棱上另一面读——另一面是 `end` / `top` / `bottom` 就直接读它，不打垂距、**不报落空**（棱本来也属于它）；另一面也是主面就沿它的法向再打一次；都不成才留第一面 + `perpendicular_ray_missed`。用户 09-18 口径：斜靠墙端竖棱的构件读 `end`，不要「`side` + 落空警告」。live 126 对里「主面 + 落空警告」由此从 24 对降到 7 对（4 对 0 mm 贴合、3 对平板棱两侧都是主面）。**贴合不打射线**（`83beef257`，用户 09-18 09:56 口径）：距离 ≤ 垂距容差 max(1%, 0.05 mm) 时直接 `perpendicular = {distance_mm: distance, from: p, to: q}`、无 warning——从贴合点出发的射线打不出 toi，打了只会被当成落空；0 mm 贴合垂距即 0，0.05 mm 内的微小间隙给距离本身而不是硬写 0。live 那 4 对 0 mm 贴合由此有了 `⊥ 0`，「主面 + 落空警告」7 → 3 对（验证 README §8.5）。相交时不算垂距。live 实测：正对墙面时 ⊥ 与最近距离相差 ≤ 0.07 mm（ELBO × 弧墙 64.43 / 64.50）；SCTN × 弧墙、BOX × 直墙的最近点落在棱 / 角上，垂距如期置空。
...
| G2 | 直墙、构件在墙端外斜靠 | 最近点落墙端面 / 竖棱，读 **`end`**、`perpendicular = null`、无 warning（09-18 `0dce7fee2`：共棱处先选法向贴合源方向的面，选中主面而射线落空就退回棱上的 `end`；只有棱两侧都是主面且都落空才留 `side` + `perpendicular_ray_missed`） | 合成单测 ✓（`g2` 500 mm 到竖棱读 `end`；`edge_hit_falls_back_to_end_when_the_side_ray_misses` 侧面先选中、射线落空 → `end` 无 warning）；live BOX × STWALL 1 同形（角上，无垂距）；126 对重放里 17 对由「`side` + 落空」改读 `end` / `top`（验证 README §8.4） |
