...
- 2026-09-15 10:0x · **U0 第四步：草稿 / 已确认分层合并 + 异步回执盖戳（截图 / 保存 / 高亮 / 相机）**（fable-5-1-38 起稿，fable-5-1-48 接手收尾）· **分层合并**：新增 `src/review/domain/draftLayerMerge.ts`（纯函数 `mergeConfirmedReplayWithLocalDrafts`：已确认层对它含有的 id 权威、本机同 id 副本一律被覆盖——记录上没有 `updatedAt`，分不出「本机后改」还是「服务端后改」，宁可以服务端为准，三方合并等 U3 的草稿快照 + `base_record_revision`；本机 id 不在已确认层的条目原样保留、排在已确认之后；只按 id 对齐、不 normalize（交给 `importJSON` 漏斗）；本机 payload 不是 V7 就只回放已确认层并标 `localPayloadIgnored`）；`confirmedRecordsRestore.ts`：`isLayeredDraftRestoreActive`（开关 `scopedDraftsV1` 开 + store 有草稿 scope + 能 `exportJSON`）为真时恢复不再整份 `importJSON` 替换，空记录任务也**不再 `clearAll`**（容器已按 scope 切好，内存里就是本任务自己的草稿；评论线程照旧清、场景照旧同步），`layerConfirmedReplayForStore` 给 `ReviewPanel.refreshEmbedSnapshot` 的可信快照刷新复用同一套判定；`layeredDrafts` 选项可显式关回旧行为。**回执盖戳**：`useAnnotationDraftSession` 新增 `isTransientReceiptCurrent(stamp)`（瞬态回执——相机 / 高亮 / 选择 / 提示 / 状态重拉：没戳放行，有戳 key 与 epoch 都得对，A→B→A 之后 A 的旧回执也拒）与 `routeDataReceipt(stamp)`（数据类回执落点 `current` / `other-scope{scopeKey}` / `unscoped`——epoch 变了但仍是同 scope 算 `current`，「记录 X 的截图」不因来回切一趟改变归属）；`useToolStore.patchPersistedAnnotationInScope(scope, type, id, patch)`（把迟到回执按 id 浅合并进**别的** scope 的 V7 容器，只读写原文、不 normalize、不碰内存；scope 就是当前作用域 / 容器缺 / 条目缺 / 写失败 → false）；新增 `src/composables/annotationReceiptRoute.ts`（`applyAnnotationReceiptByRoute` / `attachAnnotationScreenshotByRoute`：按落点写内存或出发时那个容器，截图三处与保存回滚共用一份）。四类发布点——**相机 / 高亮**：`useViewerContext.showModelByRefnosWithAck` 加 `shouldApply`，原样随 `showModelByRefnos` 事件带给 `ViewerPanel`，模型照常加载、加载完那一刻再问一次，拒了只加载不动视口（单构件飞行改为加载完再自己飞）并在回执带 `suppressed`；`ReviewPanel`（`ensureTaskComponentsLoaded` / `refreshEmbedSnapshot` / `handleTaskComponentSelect` / `locateWorkspaceAnnotation`）与 `DesignerCommentHandlingPanel.locateAnnotation` 全部过 `transientReceiptGuard`，`bindingResolve.markLoadResult` 照记（它说的是模型元素装没装上，不是某个任务的事），只有 toast 是瞬态的；**保存**：`useAnnotationReviewStateSync` 加 `shouldApply`（请求回来后拒了就一条都不写、结果标 `skipped`，回到原任务时 `watch(currentTask)` 会重拉），`useAnnotationSeveritySync` 严重度 / 标题保存失败的回滚按 `routeDataReceipt` 落点（已切任务 → 写出发时那个容器、不弹提示；A→B→A 仍回滚内存），`ReviewPanel.confirmCurrentData` 的 `clearAll` / 回放 / 状态重拉都核 `confirmStamp`（已切任务就不清别人的草稿——原任务容器里刚确认的条目与服务端记录同 id，下次回去分层恢复会被已确认层覆盖，不重复）；**截图**：`useDtxTools.captureCreatedCloudScreenshot`（云线自动截图，这几个 hunk 已随 `ec63a5b` 一并入库）、`AnnotationPanel.captureAnnotationShot`、`AnnotationInlineDetailCard.captureItemScreenshot` 三处出发前盖戳，回来按落点挂（other-scope 写进出发时容器、不弹成功 / 失败提示），两条路都找不到目标批注就删掉刚上传的附件不留孤儿（详情卡的 type / id 也在出发时定下，等待期间 props 换记录不串）· 验证：`draftLayerMerge.test.ts` 7 例、`confirmedRecordsRestore.test.ts` +5 例（分层判定 / 有记录按 id 覆盖 / 空记录不 clearAll / 显式关回旧行为 / embed 入口复用）、`useAnnotationDraftSession.test.ts` +2 例（瞬态守卫 / 数据落点）、`useToolStore.draftScope.test.ts` +1 例（迟到回执写别的容器、切回载回）、`useViewerContext.test.ts` +1、`useAnnotationReviewStateSync.test.ts` +1、`useAnnotationSeveritySync.test.ts` +2、`annotationReceiptRoute.test.ts` 6 例、`AnnotationInlineDetailCard.test.ts` +3 例（上传期间切任务 → 写 A 容器不碰 B 内存不弹提示、切回 A 载回；同任务照旧写内存；批注已不在 → 删附件）；`src/review + src/components/review + src/components/tools + src/composables` 全量 **162 文件 / 1554 例**全绿；`npx eslint` 改动 25 文件 0 错；`npm run type-check` 本次改动文件 **0 新增**（唯一 1 条基线外仍是 HEAD 既有的 `resolveLabelCollisions.test.ts`）· 已知边界 / 未做：同 id 的本机未确认改动会被已确认层顶掉（三方合并等 U3）；`AnnotationPanel` 的截图流程只由纯函数 + 详情卡组件测试覆盖，未单独起组件测试；URL 项目变化不重算 scope 的 projectId；U0 验收 Playwright（§4 V2：A 的迟到回执不能改 B）未落。
- 2026-09-15 10:3x · **U0 验收 Playwright（§4 V2）**（fable-5-1-48）· 新增 `e2e/review-u0-task-isolation.spec.ts`（`.gitignore` 的 e2e 白名单加一行）：`dtx_demo=primitives` 不依赖后端，`/api/review/attachments`（POST）与 `/api/review/annotations/*`（PATCH）用可挂起的 route 等测试放行；测试侧经 Vite 模块 URL 拿应用同一份 `useReviewStore / useAnnotationDraftSession / toastBus`。流程：旧全局容器先留一条草稿 → `ensurePanelAndActivate('review')` 装上 scope 同步（容器切到本 tab 的 session 作用域，内存为空）→ 进任务 A（写 `reviewStore.currentTask`）：面板出工作区、`annotation-unattributed-draft-notice` 显示「未归属草稿（项目 AvevaMarineSample）· 文字 1」、内存仍为空 → A 里加一条带说明的文字批注（severity general）+ 真实框选创建云线（触发自动截图上传，挂起）+ `saveAnnotationSeverity` 乐观改 principle（挂起）→ 切到 B（A 的容器已带 principle 刷进去）→ 放行两条回执：断言 B 的 `exportJSON` 字串 / 选择 / 相机逐字节不变、B 容器为空，A 的容器里云线挂上 `att-late-A`、文字严重度回滚到 general、切换之后没有任何截图 / 严重度 / 回滚提示 → 切回 A：内存载回迟到截图与回滚值，旧容器仍只有 `legacy-1`、提示仍在 · 验证：`npx @playwright/test test e2e/review-u0-task-isolation.spec.ts --repeat-each 3` 3/3 绿（每例 3–4 s，复用本机 3101 dev server）；`npx eslint` 0 错 · 边界：primitives demo 下 Viewer 忽略 `showModelByRefnos`，相机 / 高亮那一路的 `suppressed` 正向路径只能靠 `useViewerContext.test.ts` + 真机走查，这里只断言「相机 / 选择没被别的回执动过」；§3 U0 验收列的 同任务新轮次 / 登出换用户 / 新建未获 taskId / localStorage 写失败 四个变体未落；画布拾点要求 `elementFromPoint` 落在画布上（校审面板开着时批注工具条会压在画布正中）。
- 2026-09-15 10:5x · **U0 e2e 补四个变体**（fable-5-1-48）· 同一文件 `e2e/review-u0-task-isolation.spec.ts` 抽出公共步骤（`openReviewPanel / addTextDraft / createCloudAwaitingUpload / installHeldUploadRoute / installHeldSeverityRoute / startHeldSeveritySave`），新增：**同任务新轮次**——同一 `task.id` 换 `reviewRound: 1` 就是 `|round=1|` 的另一个容器、内存为空，第 0 轮挂起的自动截图放行后只落第 0 轮容器、第 1 轮 `exportJSON` 不变，切回第 0 轮截图在；**登出换用户**——`userStore.currentUserId` SJ → JH → null（anonymous）→ SJ，同任务下三个容器互不可见，SJ 出发的迟到截图只归 SJ；**新建未获 taskId**——没任务时 scope 用本 tab 的 `draftSessionId`（与 `sessionStorage['plant3d.review.draftSessionId']` 一致），草稿进 session 容器、旧全局容器没有它，挂起的严重度保存在进任务 A 之后失败 → 回滚只写 session 容器、A 的 `exportJSON` 不变，回到无任务载回，**整页刷新后同一容器、草稿不丢**；**localStorage 写失败**——`Storage.prototype.setItem` 对 `plant3d-web-tools-v7:` 前缀抛 QuotaExceededError → 状态条 `annotation-draft-status-local` 变 `write-failed`、文案「本机写入失败」+ 原因，内存有、容器没有；恢复后下一次变化回到 `saved*` 且容器补齐；出发时容器写不进去的迟到截图 → `DELETE /api/review/attachments/<id>` 删附件不留孤儿、容器与当前任务都不动 · 验证：串行 5/5、并行 `--repeat-each 2` 10/10 绿；eslint 0 错 · 观察：进空任务时状态条本机行是 `saved-unconfirmed`（「本机已存 · 未落库」）而不是 `empty`——`hasUnsavedChanges` 在 demo 下对空草稿也为 true（`buildReviewConfirmSnapshotKey` 当前 payload 与空记录 payload 不等，疑似尺寸 / 测量字段口径差），用例只断言「不是写失败」，这一条待查 · 另：本机 3101 dev server 中途换过一次进程（另一个会话 / Playwright 的 webServer 拉起的 vite 对 `backup-dbs/*.db/LOCK` 报 EBUSY watch 错误但仍在跑），跑 e2e 前先确认端口有人听。
- 2026-09-15 11:0x · **U0：工程 / URL 变化时重算 scope 的 projectId**（fable-5-1-48）· 问题：`useAnnotationDraftScopeSync` 的共享 watch 只依赖 `currentTask / currentUser / 宿主登记`，projectId 从 URL / `filesOutput.currentProjectPath` 同步读、不是响应式，`useModelProjects.applyProject` 用 `history.replaceState` 改 URL（不触发 `popstate`）、首屏那次还不派 `modelProjectChanged`，于是切工程后草稿仍写在旧工程的 scope key 下 · 改法：`src/lib/filesOutput.ts` 加 `onCurrentProjectPathChange(listener)`（`setCurrentProjectPath` 值真变了才通知，空串归一成 null）；`useAnnotationDraftScopeSync.ts` 装同步时订阅三路信号 `onCurrentProjectPathChange` / `modelProjectChanged` / `popstate` → `projectRevision +1` → 共享 watch 重取 projectId（与 `useToolStore` 刷新旧作用域用的信号一致），最后一个宿主卸了一起拆；工程换了就是另一个 scope：容器切走、epoch+1，旧工程在途的回执按既有落点归旧工程。时序：`applyProject` 里 `setCurrentProjectPath` 先于 `syncProjectUrl` 与 `refreshToolStorePersistedScope({force:true})`，我们的 `flush:'sync'` watch 在那一刻就切好容器，后面的 force 刷新与 `modelProjectChanged` 都是同 scope 幂等 · 验证：`useAnnotationDraftScopeSync.test.ts` +1 例（setCurrentProjectPath 跟上且同值不重算 / modelProjectChanged / popstate 各重取一次 / 卸载后信号不再碰 store）→ 10/10；`useToolStore.draftScope / ReviewPanel / DesignerCommentHandlingPanel / InitiateReviewPanel.minDeliveryUnit / useModelProjects / genModelE3dParquetApi / useDbMetaInfo` 全绿；`npx eslint` 0 错；`npm run type-check` 0 新增。
- 2026-09-15 11:2x · **状态条空态：空任务不再被判成「有未确认修改」**（fable-5-1-48）· 查因（临时 Playwright 诊断读组件 `setupState`）：`ReviewPanel.hasUnsavedChanges` 的草稿侧 key 多了一段 `"dimensionDocument":{"documentId":"dimension-document:local:project=AvevaMarineSample|db=__all__","records":[],"schemaVersion":2}`——视口一打开 `ViewerPanel` 就 `bindDimensionDocumentSession` 绑一份本地空尺寸文档（baseVersion 0），`getBoundDimensionConfirmPayload()` 把它带进草稿 payload，而空记录那边没有尺寸文档，两个 key 不等 → 本机行「已存 · 未落库」、「确认当前数据」常亮 · 改法：`reviewPanelActions.ts` 新增 `normalizeDimensionDocumentForComparison`——**从未落过版的空文档**（`records` 为空且 `dimensionDocumentVersion` 为 0 / 缺省）在内容比对里等价于「没有尺寸文档」，`buildReviewConfirmSnapshotKey` 与 `buildUnsavedReviewConfirmPayload` 的尺寸文档比对都走它；**已有版本号的空文档仍算内容**（`ReviewConfirmation.test` 的「尺寸文档 dirty 时提交完整快照」是版本 6 的空文档，删光已落库尺寸也得能确认下去——第一版只看 records 为空就归一，把这条测试打红了，随即改成版本感知）；确认 payload 本身不改，空文档照旧随 payload 带着 · 验证：`reviewPanelActions.test.ts` +1 例（key 相等 / 未保存差异不含尺寸文档 / 版本缺省同样 / 版本 6 的空文档仍是差异 / 基线有记录当前清空仍提交空文档）→ 28/28；`ReviewConfirmation.test` 4/4、`ReviewPanel.test` 46/46；e2e `review-u0-task-isolation` 的写失败变体把进空任务时的断言收紧为 `data-state="empty"` + 「本机 · 无草稿」→ 5/5 绿；review + tools + composables + dimension 全量绿；`npx eslint` 0 错；`npm run type-check` 0 新增。
