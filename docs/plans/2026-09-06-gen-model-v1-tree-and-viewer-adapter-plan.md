...
> **2026-09-09 深夜：P5 整条下线**（用户口径：「节点树和三维模型数据全部以 API 的数据形式提供、即时生成，不需要去用数据库里面的数据」；收口计划 §12）。删除 `useGenModelV1ModelSync.ts`、`genModelV1Ws.ts` 及各自单测，`genModelV1Api.ts` 去掉 `GET /tasks` 绑定，`showModelByRefno` 去掉 `reload` 选项与 `ViewerPanel` 的透传，徽标去掉 WS 小点与「模型同步」行。记录源的 `collectedRoots / leavesOfRoot / invalidateRoot` 作为通用缓存操作保留。下面是当时的落地记录，只作历史。
...
### 8.13 P8 两源对拍：legacy 参照在本机不成立，改记数据级对拍 + v1 浏览器/records 互证（2026-09-09）

- **环境**：gen-model `D:\Rust\target\release\aios-database.exe`（0.1.21，`:8022`，external store SurrealDB `:8009`）；legacy 按用户口径改为 **plant-server**（`D:\work\plant-code\plant-web-server`，`WEB_SERVER_PORT=3100 plant-web-server.exe --repo-root ../plant-model-gen --config db_options/DbOption-cursor`，detached runtime，静态目录 = `plant-model-gen/output`）。`verify-gen-model-v1.ps1 -Ensure` 对 0.1.21 **11/11**。
- **P8-2 浏览器两源对拍（`e2e/gen-model-v1-two-source-parity.spec.ts`）第一跑全红，逐项归因后全是 legacy 侧环境残缺，不是 v1 回归**：
  - `BRAN 24381_145018`：legacy 11 vs v1 22；`ZONE 24381_144870`：legacy 0 vs v1 2024；`EQUI 24381_109581`：legacy 0 vs v1 40；SUPPO 搜不到。
  - 根因 ①：`plant-model-gen/output/AvevaMarineSample/` **没有 `scene_tree_parquet/`**（`pdms_tree_7997.parquet` / `world_sites.parquet` 均 404），legacy 的模型树与 `visible-insts`（DuckDB-WASM 读这份树）空转 → show_refno 只装根 refno 自己：BRAN 只剩 `includeOwnedTubings` 带出的 11 根直管（截图里只有直段、没有弯头/阀），ZONE/EQUI 什么都画不出；plant-server 的 `/api/e3d/*`、`/api/pdms/ui-attr/*` 在 detached runtime 下回 `MODEL_REFNO_NOT_FOUND` / 「route not available」，属性面板「属性查询失败」同源。
  - 根因 ②：legacy parquet（`parquet/manifest_7997.json`，`generated_at=2026-08-03`）**只覆盖 ZONE 144870 的管道**：`geo_instances` 3052 行全部落在该 ZONE 内，EQUI 子树 49 个元素 0 行几何；dbnum 7997 全库 56 种 noun 里**没有 SUPPO**。
- **数据级对拍（R2 口径，DuckDB 直读 legacy parquet vs `/api/v1/model/ensure`→`records`）**：
  - `BRAN 24381_145018`：**对象数 22 = 22**（legacy 11 构件 + 11 直管；v1 11 构件项 + 11 TUBI 项），构件 refno 集合 11/11 相同；世界 AABB 角点最大差 5.2 mm（legacy 存的是逐实例保守盒，不是网格紧盒——几何本身 D6 已 ≤0.002 mm）。
  - `ZONE 24381_144870`：legacy 1287 有几何构件 + 946 直管 = 2233；v1 1341 构件项 + 683 TUBI = 2024。**构件：legacy 1287 个全部在 v1 里**，v1 多出 54 个（COUP 23 / OLET 13 / ATTA 6 / REDU 5 / GASK 4 / TEE 1 / WELD 1 / ELBO 1）——这些在 legacy 里是「有元素无几何」（旧链路 cache miss / 未支持）。**直管 −263**：逐 BRAN 看（如 `24381_145052`），legacy 在 WELD→VALV→WELD 之间按**元件原点到原点**各画一段直管（穿过阀体、弯头体），gen-model 按 arrive/leave 点画、零间隙不出段；legacy 从 ELBO 原点向两侧焊口伸出的存根直管也让它的 AABB 比 v1 多伸出 ≤96 mm（`min.x` −17458.75 vs −17362.89，极值都落在这类 TUBI 上）。即：ZONE 级差异全是 **legacy 侧的画法伪影 + 覆盖缺口**，v1 ⊇ legacy。
  - `EQUI 24381_109581`：legacy 0（无几何数据）；v1 40 = 子树里 BOX 13 / EXTR 13 / CYLI 8 / RTOR 4 / CTOR 2 全部基本体，一个不少。
  - 两条链的 `geo_hash` 空间不同（同一构件哈希不相等），不能拿哈希做对拍；几何一致性以 D6 数值对拍为准。
- **v1 浏览器 ↔ records 互证（同一 spec 第二条用例，legacy 不在也能跑）**：三类节点 **DTX 登记对象数 = `[show_refno] 加载完成` 的 loadedObjects = records 逐根条数**：BRAN 22/22/22、ZONE 2024/2024/2024、EQUI 40/40/40；bbox 有限；截图 `e2e/screenshots/v1-vs-records-{BRAN,ZONE,EQUI}-gen-model-v1.png`（树、属性、三维都在）。spec 第一条用例现在先 HEAD `scene_tree_parquet/pdms_tree_7997.parquet`，缺就带原因 skip，不再把环境残缺报成回归；两条用例都打印数据水位（legacy manifest `generated_at` vs gen-model `file_latest_sesno`）。
- **P8-3**：ZONE 全部 165 根、2024 条记录里 `is_invalid_tubi=true` **0 条**——样本缺失，告警色肉眼核对维持「未验证」。
- **P8-4a live**：`GET /api/v1/tree/children?refno=1/1` → **404 `not_found`**（`ref0 1 反查不到 dbnum`），0.1.21 上核过；P8-4b（真实 drain → WS `model_drain` → 前端重载）本轮没触发 drain，未验 → **2026-09-09 深夜改记「不适用」**：前端只吃自己 ensure 出来的即时数据，P5 同步整条下线（§8.6 头注、收口计划 §12），没有「等服务端 drain 再重载」这一路了。
- **结论**：本机 legacy 不具备做 P8-2 浏览器两源对拍的条件（树 parquet 缺、EQUI 无几何、SUPPO 无样本，且 legacy 直管画法本身有伪影）；能拿到的证据是「v1 ⊇ legacy 的构件集 + D6 样本对象数相等 + v1 浏览器/records 三类节点逐条相等」。D8（翻默认）按这组证据拍板，见收口计划 §5 / §9。
- **P8-5 翻默认（2026-09-09 晚，D8 按 B 口径落地）**：`DEFAULT_MODEL_SOURCE_KIND → 'gen-model-v1'`（`src/model-source/ports.ts`，注释记依据）；`.env.example` 与本地 `.env.development` 的注释改口（仍注释着，缺省由代码给）；`env.d.ts` / `kind.ts` / `index.ts` 头注同步；联调指南（开头加一句「缺省即 v1、回旧链路要显式写」+ §2 / §3 / §6 / §8 改口）、CONTEXT.md「模型数据源」词条、ADR 0054 追记段各一处。legacy 开关保留一个发布周期（`?model_source=legacy` / `VITE_MODEL_SOURCE=legacy`），legacy 路径代码一行未动。
  - **单测**：`src/model-source/index.test.ts` 改成钉「缺省 gen-model-v1 + legacy 两种写法都能显式回」（10 条）。翻默认后全量第一跑**新增 28 条 fail**，全部出自 5 个测 legacy 链路的文件（`useDbMetaInfo.test.ts` 4、`useDbnoInstancesDtxLoader.test.ts` 9、`useDbnoInstancesDtxLoader.invalidTubi.test.ts` 3、`useModelGeneration.loadScope.test.ts` 8、`useModelGeneration.noopWarning.test.ts` 2）——它们 mock 的是 `/api/e3d/*` / parquet / `db_meta_info.json`，此前靠「缺省 legacy」隐式成立，现在文件头显式 `window.history.replaceState('?model_source=legacy')` 钉回（它们测的就是旧链路形状，不是缺省值）。钉回后全量 **1977（+1）/ 1952 passed / 25 failed**，失败集 ⊆ 同机同刻的 HEAD 基线（1976 / 1946 / 30），**新增 fail 0**（中间一跑另见 review 域 5 条 5 s 超时型偶发，单跑全绿、与本改动无关——本机同时有别的会话的 gen-model 实例在算）。
  - **浏览器路由钉子** `e2e/model-source-default.spec.ts`（入库，`.gitignore` 白名单；不依赖任何后端在跑，只看请求发往哪里）：`/` → `getModelSourceKind()==='gen-model-v1'`，首批取数 `/api/v1/health` + `/api/v1/tasks?kind=model_drain` + `/api/v1/dbnums` 3 发、旧链路（`/api/e3d/*`、`/api/pdms/*`、`/files/output/**`、`/files/meshes/**`）**0 发**；`/?model_source=legacy` → `'legacy'`，`/files/output/AvevaMarineSample/scene_tree/db_meta_info.json` + `scene_tree_parquet/world_sites.parquet` 2 发、`/api/v1` **0 发**；**2 passed**。ESLint 触及文件 0 错。
  - ~~**未验证**~~ **已补齐（2026-09-09 深夜）**：`:9099` 上那份 0.1.21 出厂包到 22:5x 已有数据（`/dbnums` 有行、`model_publish_queue` generated 2908），Node 探针 `GEN_MODEL_V1_BASE_URL` / 页面 `VITE_GEN_MODEL_V1_BASE_URL` 指到 9099 后一并跑 `model-source-default`（2/2）+ `gen-model-v1-mesh-direct`（1/1，`.mesh` 10/10、`.glb` 0）+ `gen-model-v1-two-source-parity`（用例一按设计 skip；用例二过：BRAN 22=22=22、ZONE 2024=2024=2024、EQUI 40=40=40）——**4 passed / 1 skipped**。细节见收口计划 §11。
...
- **仍欠**：~~默认开关翻到 `gen-model-v1`~~ **已翻**（2026-09-09 晚，收口计划 D8 按 B 口径，见 §8.13 末条；legacy 开关保留一个发布周期，到期后再评估下线 legacy 适配器）；`is_invalid_tubi` 告警色的浏览器肉眼核对（§8.12，要一条真的无效直管）；gen-model 侧按库 / 多根批量 `records`（要先定契约）。`tree/*` 的 Ref0-不在-MDB 分型**已落地**：gen-model `97e819a5a`（2026-09-08，`ApiError::from_domain` 把 `DirectStoreError::UnresolvedRef0` 与 `NoSuchElement` 同码 404 `not_found`，spec §4.10 补一条，`cargo test --lib -- web_service` 40 passed；未 live）。`model_drain` WS 事件**已落地**：gen-model `80b0f330c`（2026-09-08）——发送点绕开一直有人在改的 `model_update_pending.rs`，放在 `task_registry` 的 `insert_running_model_drain` / `finish`（drain 的认领与收口只经这两个口）：`task_started {task_id, kind:"model_drain", project, total_units, detail}`、`task_finished {task_id, kind, project, state, result, detail}`，`detail.roots[].target_refno`（`a/b`，≤10 条样本 + `root_samples_truncated`）就是 §8.6 要的根列表；数据批次 / 房间轮不受影响（注册表只对 drain 发）。spec §5.3 同步；`cargo test --lib -- task_registry` 16 passed（+2）、`-- web_service` 40 passed；**未 live**（本机此刻没有 gen-model 实例在听）。前端 `useGenModelV1ModelSync` 今天对任何 `task_finished` 都触发一次 REST 对齐，所以换上这版服务端后 drain 收口 → 立刻对齐已经成立，不必改代码；后续可选：按 `payload.kind === 'model_drain'` 直接取 `detail.roots` 重载、把 15 s 轮询拉长到分钟级。
- **仓级发现**（2026-09-08，见 §8.12）：`npm run type-check` 空转，真查用 `npx vue-tsc --build --force`（全仓 642 条既有错误）。——**已修**（2026-09-09，收口计划 P10-1 / §11）：`npm run type-check` 现在真查并与 `scripts/type-check-baseline.txt`（646 条基线）比对，只对新增错误报红。
