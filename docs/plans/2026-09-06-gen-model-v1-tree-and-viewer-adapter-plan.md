...
- **P8-5 翻默认（2026-09-09 晚，D8 按 B 口径落地）**：`DEFAULT_MODEL_SOURCE_KIND → 'gen-model-v1'`（`src/model-source/ports.ts`，注释记依据）；`.env.example` 与本地 `.env.development` 的注释改口（仍注释着，缺省由代码给）；`env.d.ts` / `kind.ts` / `index.ts` 头注同步；联调指南（开头加一句「缺省即 v1、回旧链路要显式写」+ §2 / §3 / §6 / §8 改口）、CONTEXT.md「模型数据源」词条、ADR 0054 追记段各一处。legacy 开关保留一个发布周期（`?model_source=legacy` / `VITE_MODEL_SOURCE=legacy`），legacy 路径代码一行未动。
  ...
  - **未验证**：翻默认后带 gen-model 在跑的整链——本轮 `:8022` 没有实例（上一会话起的进程随其终端回收；`:9099` 上是别的会话装的 `D:\ams-local-0.1.21` 出厂包，`model_ready=false`、`/dbnums` 0 行，不能当参照）。gen-model 起来后跑一次 `e2e/gen-model-v1-two-source-parity.spec.ts` 第二条用例即补齐（用例本身不依赖 `?model_source=` 缺省值，翻前翻后口径一致）。
...
- **仓级发现**（2026-09-08，见 §8.12）：`npm run type-check` 空转，真查用 `npx vue-tsc --build --force`（全仓 642 条既有错误）。
