...
- **要用户看一眼的口径后果**：rule (a) 让「偏主面法向 ≤ ~8°（1% 容差）就读主面 + ⊥」成了通则，live 里比目标 3 对多带出 21 对。要是想让有非主面可退的仍先读 `end` / `top`、只在没得换面时才取 edge 垂距，把 (a) 挪到 (b) 之后即可（一处顺序）；那样目标 3 对会跟 §8.4 那 17 对一样读 `end`、无 ⊥。`:8022`（`994f2e0cc`）/ `:8023` 未动。
