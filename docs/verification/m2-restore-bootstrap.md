...
1. Seed the data.
2. Obtain a valid `user_token` for a reviewer embed session.
3. Open the app with the route above, replacing `<token>`.
4. Confirm workflow-sync restore resolves `FORM-M2-EMBED-001` and replays the seeded cloud annotation.
