...
> 2026-09-16 落成；差异见 §7。**D1 已拍板并改掉（用户 10:54，`0c0d9eb`）**：两线夹角提示按 E3D `stdGraphics` 口径去掉 `(token)` 与 ` Snap` 尾巴；D5 的 Web 文案同笔改掉，E3D 原文仍待采；D6 静态核清结掉；**D2 用户 2026-09-16 15:5x 改口、改掉（`e8ebc10`，决策 `d-269` 取代 `d-228`）**：Fraction 提示照 E3D 原样出输入值；**2026-09-17 06:4x 实机翻案**（golden MD §40）：E3D 用户在输入框填 2.5 实际出 `Fraction[3]` 落三分点，`Fraction[2.5]` 只有程序直写值才出——**D2 待重新拍板**（选项见 §7）；余 D4（不做）。
...
| 4 | `Fraction` | `Fraction[<pickTypesValue[4]>]`（1889，原样拼 `pickTypesValue[4]`——但**实机填 2.5 出 `Fraction[3]`**：pad 输入框 `is REAL format !!edgFormat`，Fraction 时 `!!edgFormat = !!integerFmt`（dp 0），`setInput` 取 `gadget.val` 已是**四舍五入后的整数**；`Fraction[2.5]` 只在程序直接写 `pickTypesValue[4] = 2.5` 时出现） | 缺省 `2`，输入框（**E3D 3.1 的 pad 没有 Fraction 按钮**，`edgnewpositioning.pmlfrm` 55–56 / 128–129 注释掉，UI 进不去） | Fraction | `Fraction[<fraction>]`（原样，`formatPromptReal`；`e8ebc10` 前是 `Fraction[<trunc(fraction) ≥ 1>]`） | ~~一致（**D2 已改 `e8ebc10`**）：输入 `2.5` 两边都出 `Fraction[2.5]`~~ **实机不一致（golden §40，2026-09-17 06:4x，带截图）**：E3D 用户能走到的路径（pad 输入框敲 `2.5` Enter）→ 提示 `Fraction[3]`、输入框重显 `3`、内核 `int(3)` → **三分点**（真实拾取落在 2/3，`ratio=0.666667`）；Web 现在 → `Fraction[2.5]`、回显 `2.5`、`int(2.5)=2` → 二分点。E3D 只有绕过输入框直接写 `pickTypesValue[4]=2.5` 才出 `Fraction[2.5]`（此时输入框仍重显 `3`、落点 1/2，`s5` / `s6` 实测 Dist 391.54 mm = 2349.23 × (2/3 − 1/2)）。→ **D2 待重新拍板** |
...
**D2 2026-09-17 06:4x 实机翻案（golden MD §40）**：七种 token 字面 ✓，但用户在 E3D pad 输入框填 `2.5`，E3D 出的是 `Fraction[3]`、回显 `3`、落三分点（gadget 按 dp 0 回读）；`Fraction[2.5]` 只有程序直写值才出现，且 3.1 的 pad 没有 Fraction 按钮。Web 现在填 2.5 → `Fraction[2.5]` 落二分点，与 E3D 用户路径**两处都不同**——**待用户重新拍板**（选项见 D2 行）。
