...
> **证据等级**：E3D 原为一律 `static_expectation`（字串照源码抄）；**2026-09-16 23:3x 起**，§2 四条命令的第 1 步、§3 七项顺序、§4 八项顺序、§5 三条尾巴的缺省状态已在跑着的 E3D 3.1（shadow，PID 11900）里用 `capture-prompt-step.pmlmac` 直读 `!!edgCntrl.state.prompt()` 与视口 inMode prompt 采成 `observation`（golden MD §38，`e3d-prompt-capture.trace.txt`，无截图——主窗口最小化）；**2026-09-17 00:1x / 06:4x**（golden MD §40）：§3 七种 token 的实串、**D2 三处**（用户在 Positioning Control 里填 2.5 → 提示 / 输入框 / 落点，**实机是 `Fraction[3]` / `3` / 三分点**，见 §3 第 4 行与 D2）、§2 两线夹角第 2 步与距离第 2 步的步词也采成 `observation`——这一轮主窗口已还原（用户 00:00 点头），真实鼠标拾取 + 键盘输入，**带截图**（`e3d-prompt-P3-04-r2-*.png` / `e3d-prompt-P2-04-r2-*.png`）；其余行仍 `static_expectation`；Web 字串照源码抄，并与 golden MD §35 / §30 补采实机截图里的提示条核过（`(Distance[100]) Snap : SCTN PLINE NA · Distance[100]`、
...
| | 2 `end` | `Measure distance end (Snap) Snap :` | 距离测量 · 选择终点 | `距离测量 · 第 2/2 步 选择终点 (Snap) Snap : …；点空白取消当前点选` | 一致；trailer 是 Web 加的；**步词 `end` E3D 实机 ✓**（golden §40 `P3-04-r2-s4` / `s5`：真实拾中第一点后 `Measure distance end (Fraction[3]) Snap :` / `… end (Fraction[2.5]) Snap :`——token 位当时是 Fraction，`(Snap)` 变体没单独采；第一点处 E3D 挂一枚 AID TEXT `Measure distance start`，两点齐后 Measure 窗体出 Dist 且状态回 `start`） |
