...
> 2026-09-16 落成；差异见 §7。**D1 已拍板并改掉（用户 10:54，`0c0d9eb`）**：两线夹角提示按 E3D `stdGraphics` 口径去掉 `(token)` 与 ` Snap` 尾巴；D5 的 Web 文案同笔改掉，E3D 原文仍待采；D6 静态核清结掉；**D2 用户 2026-09-16 15:5x 改口、改掉（`e8ebc10`，取代决策 `d-228`）**：Fraction 提示照 E3D 原样出输入值；余 D4（不做）。
...
**D2 2026-09-16 15:5x 用户改口、改掉（`e8ebc10`，新决策取代 `d-228`）**：Fraction 提示照 E3D 原样出输入值（`Fraction[2.5]`），落库不再归一，内核仍 `int()`——七种 token 现在字字与 E3D 相同。
