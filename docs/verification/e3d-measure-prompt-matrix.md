...
顺手观察（golden MD §40.8）：E3D pad 在 Ppoint / Screen 过滤器下把五颗拾取类型按钮置灰（`edgnewpositioning.pmlfrm` 200 `active = not inset(5,6)`），提示里的 `(Snap) Snap` 与 `pickTypeIndex` 不变；Web 浮条没这条联动（拾取类型的 `disabled` 只看静态 `MEASUREMENT_PICK_TYPE_AVAILABILITY`）——不进提示，属 pad UI 差异，不另立 D 项。
