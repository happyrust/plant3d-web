...
**仍留**：三维 framed 文字（沿尺寸线的数字）不吸格——透视下笔画不与屏幕轴对齐，只受益于 sRGB 混合与免 FXAA；~~`dimensionStrokeWidthPx` 1.2 px 的尺寸线在 1× 上实心核仍只有 0.2 px（现在按 sRGB 混合看起来实一些，但没有取整），要不要给线也定「最少 1 个设备像素整宽」另拍~~（下一小节，d-537）；SVG 导出与命中区仍用名义 1.8 px。

### 尺寸描边取整到设备像素、最少 2 个（2026-09-14 21:1x–21:5x，fable-5-1-26，ADR 0064「补」；用户「给尺寸线也定「最少 1 个设备像素整宽」：先在 1× / 2× 实机量 1.2 px 尺寸线的实心核，再照 ADR 0064 的取整口径改并另起决策」）

**先量（改代码前）**。同一条 BRAN `24383_67485`、同一套相机（`pw-petags-17.mjs`），但 vite 起在 HEAD `1f44f0c` 的**独立 git worktree** `:3103` 上——工作树 `:3101` 正被别的会话边改边存，HMR 一刷页面模型就没了（第一趟 mid / elbo-mid 全空，2× 那趟在 `flushLayout` 撞 null），换 worktree 后 far / mid / elbo-mid 读数与 fable-5-1-17 那趟逐个相同。量法（`dimline-pixels-26.py`）：取只含一条图纸红 `#c81e1e` 尺寸线的框（`630` 竖线 185 行 / `500`·`250.01` 斜线 ~160 列），每个横截面里每个像素的覆盖率由**绿通道对背景的下降**反解（叠层现在按 sRGB 混合，各通道都是线性插值），红通道跟着绿一起掉的（灰网格 / 深色文字 / 蓝管）剔除；峰值 = 横截面最大覆盖率，实心核 = 覆盖率 ≥ 0.98 的像素，墨量 = 覆盖率之和按倾角修正 = 有效宽度（设备像素）。

| 变体 | DPR | 峰值覆盖率均值 | 有实心核的横截面 | 核 px / 横截面 | 墨量（设备 px） |
| --- | --- | --- | --- | --- | --- |
| 修前 1.2 px 不取整 | 1× | 0.847 | **25 %** | 0.25 | 1.22 |
| 取整、下限 1 → 1 设备像素 | 1× | 0.758 | **4 %** | 0.04 | 1.02 |
| **取整、下限 2 → 2 设备像素（采用）** | 1× | **1.000** | **100 %** | 1.03 | 2.02 |
| 修前 1.2 px = 2.4 设备像素 | 2× | 1.000 | 100 % | 1.45 | 2.42 |
| 取整（两种下限同为 2.4 → 2）| 2× | 1.000 | 100 % | 1.04 | 2.02 |

（`630` 竖线；`500` 斜线同趋势：1× 25 % → 4 % → 96 %，2× 95 % → 93 %。`dimension-line-core-sweep.json` 两框全数。）读数与理论一致：核宽 = 宽 − 羽化 = 1.2 − 1 = 0.2 设备像素，横截面中心落进去的概率 ≈ 20 %；1 个设备像素时核 0；2 个时核 1。

**改法**（`scenePainter.ts`）：`hintedLineStrokeWidthPx(w, dpr)` = max(2, round(w · dpr)) / dpr，所有不带 tag 色调的默认描边（尺寸线 / 尺寸界线 / 引线 / 标记）走它并置 `pixelSnap = 1`（轴对齐的线两端吸格，透视里的斜线不受影响）；tag 边框 / 引线与 framed 三维文字不动；命中区与 SVG 仍名义 1.2 px。**下限取 2 不取 1**：下限 1 在 1× 上是倒退（上表第二行——比修前更淡、更细，肉眼可见），下限 2 才给每个横截面一个满覆盖像素。代价是 1× / 1.25× 上线与文字同重（2 个设备像素）；1.5× 起文字仍更重（3 vs 2、2× 4 vs 2）。

![1× 250.01 尺寸：修前 1.2 px | 下限 1 → 1 px | 下限 2 → 2 px，3× 逐像素](dimension-line-1x-250-before-min1-min2.png)

![1× 630 竖线 6× 逐像素：修前 | 下限 1 | 下限 2](dimension-line-1x-630-before-min1-min2.png)

![2× 250.01 尺寸：修前 2.4 设备像素 | 取整 2 设备像素，3×](dimension-line-2x-250-before-after.png)

**单测**：`vitest run src/dimension src/composables/useMbdExternalSync.test.ts` **59 文件 / 363 通过**（+1 `hintedLineStrokeWidthPx` 表；改写：尺寸线 / 引线 / 标记 `pixelSnap` = 1 且 1.5× 下 1.33 CSS px、tag 边框仍不吸不取整、1× 下第一段 2 px）；eslint 0；type-check 基线外 52 条全在别的会话正在改的测量 / 云线 / 校审文件里，`src/dimension` 0 新增。实机 after 两档 DPR 各跑两遍（下限 2 的 worktree 副本与最终提交文件）截图逐字节相同。

| 新增文件 | 说明 |
| --- | --- |
| `dimension-line-core-sweep.json` | 修前 / 下限 1 / 下限 2 × 1× / 2× × 两条线的横截面统计（峰值、核、墨量、覆盖率直方图） |
| `dimension-line-1x-250-before-min1-min2.png` | 1× `250.01` 尺寸（线 + 三维数字 + 箭头）三联 3× |
| `dimension-line-1x-630-before-min1-min2.png` | 1× `630` 竖线三联 6× |
| `dimension-line-2x-250-before-after.png` | 2× `250.01` 修前 / 修后 3× |

**仍留**：~~tag 边框（1 px）/ 引线（0.9 px）同样没有实心核也不吸格——边框是屏幕空间的轴对齐矩形，吸格收益最直接，要不要一并另拍~~（下一小节，d-539）；1× 上线与文字同重，若嫌线粗只有回到下限 1（更淡）或保留名义 1.2（无核）两条路，都要另起决策 supersede d-539。

### 标签的边框 / 方框 / 引线也取整到设备像素、最少 2 个、直边吸格（2026-09-14 21:5x–22:1x，fable-5-1-26，ADR 0064「补二」；用户「把 tag 边框（1 px）与 tag 引线（0.9 px）也纳进同一口径：取整到设备像素、最少 2 个、轴对齐的边框吸格，照这轮的法子先量再改」）

**先量**。同一条管、同一相机、同一个 HEAD worktree `:3103`。三种 tag 描边：位号方框 `R422.010-SV`（`tag-frame` 1.2 px `#0f172a`，四条边各取一段直边：顶 / 底各 88 列、左 / 右各 18 行）、端点坐标卡片 `X 11772 / Y −469 / PE 2911`（`tag-border` 1 px `#94a3b8`，四条边 52 / 52 / 46 / 46）、方框到管口的引线（`tag-leader` 0.9 px `#64748b`，32 行，倾 8°）。量法比上一节多一步（`stroke-pixels-26.py`）：描边色是参数，**两侧背景分开估**（边框一侧是白色标签体 255、另一侧是灰场景 ≈ 233），每像素覆盖率取各高对比通道反解值的均值、通道不一致的像素（文字、管、别的描边）剔除。

| 描边 | DPR | 峰值覆盖率 | 有实心核的横截面 | 核 px / 横截面 | 墨量（设备 px） |
| --- | --- | --- | --- | --- | --- |
| 方框 1.2 px 修前 | 1× | 0.83（顶 0.91 / 底 0.79 / 左 0.61 / 右 0.88） | **0 %** | 0 | 1.20 |
| 卡片边框 1 px 修前 | 1× | 0.72（顶 0.51 / 底 0.87 / 左 0.58 / 右 0.91） | **1 %** | 0.01 | 1.01 |
| 引线 0.9 px 修前 | 1× | 0.70 | **0 %** | 0 | 0.95 |
| **方框修后** | 1× | **1.00** | **100 %** | **2.00**（吸格后整 2 行） | 2.00 |
| **卡片边框修后** | 1× | **1.00** | **100 %** | **2.00** | 2.03 |
| **引线修后** | 1× | **1.00** | **100 %** | 1.03 | 2.05 |
| 方框 2.4 设备像素 修前 → 2 吸格 | 2× | 1.00 → 1.00 | 100 % → 100 % | 1.08 → 2.00 | 2.41 → 2.01 |
| 卡片边框 2 设备像素 修前 → 2 吸格 | 2× | 1.00 → 1.00 | 100 % → 100 % | 1.02 → 2.02 | 2.04 → 2.04 |
| 引线 1.8 设备像素 修前 → 2 | 2× | 0.99 → 1.00 | 84 % → 100 % | 0.84 → 1.11 | 1.89 → 2.09 |

修前 1× 的要害不只是淡：同一个方框四条边卡在**不同的亚像素相位**，顶边 0.91、左边 0.61——一个框四条边深浅不一；卡片顶边 0.51 正好对半分在两行，是两条灰线。吸格之后每条直边整 2 行满覆盖，四条边一样重。

**改法**（`scenePainter.ts`）：除 framed 三维文字（及其白边）之外的**每一条**屏幕空间描边都提示——字形段 `hintedTextStrokeWidthPx`、其余（尺寸描边 + `tag-border` / `tag-frame` / `tag-leader`）`hintedLineStrokeWidthPx`（下限 2）并 `pixelSnap = 1`；卡片 / 方框的直边是屏幕空间轴对齐段，两端吸格；圆角弧段与斜引线不动（弧段与吸格直边在角上差 ≤ 0.5 设备像素，圆头盖住，4× 图里看不出台阶）。theme 的 1 / 1.2 / 0.9 不改——名义值，命中区与 SVG 仍用它们。代价：1× / 1.25× 上位号方框的深色边框重了一档（与文字同重）。

![1× 位号方框 + 引线 4×：修前 | 修后](tag-stroke-1x-frame-before-after.png)

![1× 坐标卡片边框 4×：修前 | 修后](tag-stroke-1x-card-before-after.png)

![2× 方框 / 卡片 2×：修前 | 修后（差别只在边整齐、引线略重）](tag-stroke-2x-frame-card-before-after.png)

**单测**：59 文件 / 363 通过（`scenePainter.test.ts` 改写：tag-border 路径与 tag-leader 线 `pixelSnap` = 1、1.5× 下 1.33 CSS px、1× 下 2 px；`hintedLineStrokeWidthPx(1, 2)` = 1、`(0.9, 2)` = 1、`(0.9, 1.25)` = 1.6）；eslint 0；type-check `src/dimension` 0 新增。实机 after 1× / 2× 各一趟（`tags-probe.json` 三视图读数与 before 逐个相同，pageerror 0）。

| 新增文件 | 说明 |
| --- | --- |
| `tag-stroke-core-sweep.json` | 修前 / 修后 × 1× / 2× × 九段描边（方框四边、卡片四边、引线）的横截面统计与分组汇总 |
| `tag-stroke-1x-frame-before-after.png` | 1× 方框 + 引线 4× 前后 |
| `tag-stroke-1x-card-before-after.png` | 1× 卡片边框 4× 前后 |
| `tag-stroke-2x-frame-card-before-after.png` | 2× 方框 / 卡片 2× 前后 |

**仍留**：三维 framed 文字（沿尺寸线的数字）与白边仍不吸格、不取整——透视下不与屏幕轴对齐；1× 上所有描边（文字 / 尺寸线 / 边框 / 引线）同为 2 个设备像素，层次只靠颜色分，若要恢复「文字比线重」只能等 1.5× 及以上或改文字到 3 px（另拍）。
