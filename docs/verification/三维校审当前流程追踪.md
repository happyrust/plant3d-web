...
- `workflow_mode` 当前默认落到 `external`；只有显式切到 `manual` / `internal`，前端才会走内部提资与内部审核按钮链路。
- 设计侧默认动作是“保存提资单数据”，不是“在 `plant3d-web` 内部直接推进完整审批流”。
- 审核侧默认动作是“恢复上下文 + 展示状态 + 辅助校审”，真实流转由外部平台通过 `workflow/sync` 驱动。
...
- 设计侧提资面板：`src/components/review/InitiateReviewPanel.vue`
...
3. 真正的“送审提交”动作由外部平台侧继续执行 `workflow/sync active`。
...
4. `ReviewPanel.vue` 在 passive 模式下只显示：
   ...
   - 刷新按钮
   - 辅助校审 / 数据同步 / 历史记录
5. 在默认 external 模式下，`ReviewPanel` 不提供内部“提交到审核 / 驳回到设计”按钮链路。

### 5. 内部 manual / internal 流程仍然存在，但不是默认生产路径

1. 设计侧在 manual / internal 模式下会显示校核人、批准人、优先级、期望完成时间。
...
| 工作流角色 | 当前落点 | 主面板 | 默认行为 |
| --- | --- | --- | --- |
| `sj` | designer | `initiateReview` | 保存提资数据，等待外部平台继续流转 |
...
- external 模式下，前端内部任务更像“承载三维数据、附件、校审记录的本地镜像”。
...
如果不区分，容易把 `DesignerTaskList`、`ReviewerTaskList`、前端内部提交按钮误认为生产主路径。
...
- 当前默认就是 `external`。
- 如果测试目标是前端内部提交链路，必须显式切到 `manual` 或 `internal`。
...
3. 设计侧在 `InitiateReviewPanel` 保存提资数据。
4. 回 PMS 继续做 `workflow/sync active`。
