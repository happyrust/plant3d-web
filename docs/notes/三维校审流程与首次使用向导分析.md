...
| 节点代码 | 节点名称 | 对应角色     | 主要职责             |
|----------|----------|--------------|----------------------|
| `sj`     | 编制     | 设计师       | 发起编校审、选构件、填表单 |
| `jd`     | 校对     | 校对人员     | 校对构件、确认数据、确认流转或驳回 |
| `sh`     | 审核     | 审核员       | 审核校对结果、确认流转或驳回 |
| `pz`     | 批准     | 批准人/管理员 | 最终批准或驳回       |

流转顺序：**sj → jd → sh → pz**。可驳回至任一前置节点（如 驳回到编制、驳回到校对）。

### 1.2 角色与任务入口

| 角色         | 入口行为                         | 主要面板                     |
|--------------|----------------------------------|------------------------------|
| 设计师       | `panel.initiateReview` → 发起编校审 | `initiateReview`、`myTasks`、`resubmissionTasks` |
| 校对人员     | `review.start` → 待审核任务      | `reviewerTasks`、`review`    |
...
flowchart TD
    subgraph 设计师
        D1[InitiateReviewPanel 发起编校审]
        D1 --> D2[选构件 + 填表单 + 选校对/审核人]
        D2 --> D3[创建编校审单并确认流转至校对]
        D3 --> D4[任务进入 submitted]
    end

    subgraph 校对人员
        J1[ReviewerTaskList 待校对任务]
        J2[ReviewPanel 查看任务 + 批注/测量]
        J3[确认数据 addConfirmedRecord]
        J4[确认流转至审核 / 确认驳回流转到编制]
        D4 --> J1
        J1 --> J2 --> J3 --> J4
    end
...
1. **发起端**：设计师在 `InitiateReviewPanel` 中选构件、填表单、选校对人员/审核人员，创建任务后自动执行 `submitTaskToNextNode`，任务进入 `submitted`。

2. **流转端**：
   - `submitTaskToNextNode`：确认流转到下一节点（sj→jd→sh→pz）。
   - `returnTaskToNode`：确认驳回流转到指定前置节点。
   - 驳回后 `currentNode` 回退，设计师/校对人员在 `ResubmissionTaskList` 或 `DesignerTaskList` 中可见被驳回任务。

3. **数据确认**：
   - 校对/审核人员在 `ReviewPanel` 中使用批注、测量工具检查模型。
   - 点击「确认数据」将批注与测量持久化为 `ConfirmedRecord`。
   - 流转决策基于这些确认记录与工作流历史。

4. **角色分流**：
   - `panel.initiateReview` 用于设计师发起编校审，打开 `initiateReview`。
...
**设计师向导（8 步）**：
1. 校审功能区 → 2. 发起编校审 → 3. 填写编校审信息 → 4. 选择构件 → 5. 上传附件 → 6. 提交编校审 → 7. 我的编校审 → 8. 复审任务
