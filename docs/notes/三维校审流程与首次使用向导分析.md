...
| 角色         | 入口行为                         | 主要面板                     |
|--------------|----------------------------------|------------------------------|
| 设计师       | `review.start` → 发起提资        | `initiateReview`、`myTasks`、`resubmissionTasks` |
...
4. **角色分流**：
   - `review.start` 按角色打开不同面板：设计师→`initiateReview`，校核/审核/批准→`reviewerTasks`。
...
**设计师向导（8 步）**：
1. 校审功能区 → 2. 开始校审 → 3. 填写提资信息 → 4. 选择构件 → 5. 上传附件 → 6. 提交提资 → 7. 我的提资 → 8. 复审任务
