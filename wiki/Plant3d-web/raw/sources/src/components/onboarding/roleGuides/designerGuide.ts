...
export function buildDesignerGuide(ctx: GuideContext): GuideDefinition {
  ...
  if (isRibbon) {
    steps.push({
      ...
      title: '校审功能区',
      description: '设计相关的编校审、进度跟踪和复审入口都在「校审」标签页中。',
      placement: 'bottom',
      menuMode: 'ribbon',
    ...
    steps.push({
      ...
      title: '打开编校审入口',
      description: '点击「发起编校审」进入设计师工作台，开始整理本次需要送审的三维内容。',
      placement: 'bottom',
      menuMode: 'ribbon',
      actionHint: '请先点击顶部「校审」标签页展开功能区，再找到「发起编校审」按钮。',
    });
  }

  // 分层菜单：先从「校审」子菜单进入「发起编校审」（展开步骤由 withHierarchicalMenuCommandSteps 插入）
  if (!isRibbon) {
    steps.push({
      id: 'menu-open-initiate-review',
      targetSelector: '[data-command="panel.initiateReview"]',
      title: '菜单：发起编校审',
      description: '在顶部「校审」菜单中点击「发起编校审」，打开设计师工作台。',
      placement: 'bottom',
      actionHint: '请先展开「校审」菜单，再点击「发起编校审」。',
  ...
  steps.push(
    {
      ...
      title: '填写编校审信息',
      description: '在这里填写编校审包名称、说明，并指定校核员与审核员，让流程准确流转。',
      placement: 'left',
      actionHint: '请在右侧面板中找到「发起编校审」标签页，或通过菜单「校审 → 发起编校审」打开。',
      onBeforeShow: () => ensurePanelAndActivate('initiateReview'),
    },
    {
      id: 'select-component',
    ...
    },
    {
      ...
      placement: 'left',
      canSkip: true,
      actionHint: '请在「发起编校审」面板中向下滚动，找到「模型附件」上传区域。此步骤可跳过。',
      onBeforeShow: () => ensurePanelAndActivate('initiateReview'),
    },
    {
      id: 'submit-review',
      ...
      placement: 'top',
      actionHint: '请在「发起编校审」面板底部找到「保存编校审单数据」按钮。需要先填写数据包名称并添加构件。',
      onBeforeShow: () => ensurePanelAndActivate('initiateReview'),
    },
  );

  // "我的编校审" — 仅 active workflow 下可用
  if (isActiveWorkflow) {
    steps.push({
      id: 'my-tasks-panel',
      targetSelector: '[data-command="panel.myTasks"]',
      title: '查看我的编校审',
      description: '在「我的编校审」里跟踪已发起任务的状态、附件和流转进度。',
      placement: isRibbon ? 'bottom' : 'left',
      requiresActiveWorkflow: true,
      actionHint: isRibbon ? '请点击「校审」标签页，找到「我的编校审」按钮。' : '请先展开「校审」菜单，再点「我的编校审」；或在右侧面板打开对应标签页。',
  ...
  return {
    ...
    description: ctx.workflowMode === 'external'
      ? '了解如何在外部流程模式下发起编校审并跟踪进度。'
      : '了解如何发起编校审、跟踪进度，并在驳回后完成修改与复提。',
